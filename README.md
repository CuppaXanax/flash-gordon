...
The sole runtime uses expert parallelism: the coordinator executes the common
graph and dispatches routed experts to their owning blades. Pipeline parallelism
...
full prompt. New packs default to native 262,144-token context with a bounded
record cache and 128-token prefill chunks. Existing packs retain their sealed
budgets until upgraded. Tokens stream directly to
...
as 262,144/262,144/0/16 MiB; omitted runtime options use the sealed budgets.
...
For a quick eight-blade bring-up, smoke benchmark and terminal chat from Windows:

```powershell
pwsh -File .\tools\start-smoke-chat.ps1
```

This builds the current local working files with ordinary `make`, distributes
identical binaries/shaders to 192.0.2.42–49, creates a native-context manifest
copy beside the existing weights, replaces your running Flash Gordon processes
for that pack, and starts the API. It runs two short requests, reports prefill
and decode TPS, verifies prefix reuse, then opens a chat prompt. `/clear` starts
fresh; `/quit` leaves the server running. Logs and results go under `results/`.

It requires PowerShell 7.4+, the sibling `bc-250-dbg/Invoke-BC250Fleet.ps1`
runner and existing EP weights on all eight blades. Authentication uses that
runner's `.env`/password flow, including `FLEET_USER`; credentials are not copied
into this project. Commands and SFTP transfers share that authentication.
Defaults are pack directory
`/home/user/flash-gordon-q38-cooked`, and input `manifest-native-262k.fgm`.
Override these with `-PackDirectory` and `-ManifestName`. `-SshUser` overrides
the fleet user; `-Auth Key -IdentityFile ...` explicitly selects key authentication.
The original manifest is preserved; retired pipeline manifests
are rejected. Build dependencies must already be installed on blade 42.

If the API is already running, use `-SmokeOnly` to skip deployment. Add `-NoChat`
to exit after the smoke check. Other clients can use base URL
`http://192.0.2.42:8080/v1` and model `Qwen3.8-Flash-Next`. This quick check
does not measure long-context performance.
...
native-262k-microbatch-128` migrates the manifest without rewriting weights.
...
Default packing seals a 262,144-token logical/index context, a unified 16 MiB
record cache and a 128-token microbatch. An existing 8k pack needs an explicit
manifest upgrade; packing a new artifact does not modify it. Compared with the
legacy 256-token chunk, the native configuration halves the working chunk, so
...
QSA prefill scores bounded causal query tiles, deduplicates cold-page fetches,
and gathers private selected-record slices before cache eviction. Decode checks
the same bounded cache. New records remain pinned until the replication queue
accepts their copies. Fetches drain queued sends and owner writes before reading;
these are visibility barriers, not crash-durability guarantees. A failed session
is reset and replayed, or the runtime must be reopened if transport is poisoned.
...
Native 262,144-token support remains the product requirement. No microbatch or
context sweep is required for each change. Rank startup rejects corrupted
manifests, topology drift, protocol mismatches and memory-cap violations.
...
also needs the eight blades to run the same release build. Interpret the harness
alongside the 128k run and the current requirements in [PRD.md](PRD.md).
