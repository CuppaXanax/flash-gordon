...
/* Buffered read on a raw descriptor: no fixed-file or fixed-buffer slot is
 * consumed, so an advisory read-ahead can share a ring without disturbing the
 * blocking read batches that own the fixed resources. */
fg_status fg_uring_prep_read(fg_uring *ring,int fd,void *buffer,uint32_t bytes,uint64_t offset,uint64_t tag,fg_error *err);
...
/* Non-blocking completion drain: copies whatever the CQ already holds and
 * returns zero when nothing has completed; never enters the kernel to wait. */
fg_status fg_uring_peek(fg_uring *ring,fg_uring_cqe *out,uint32_t capacity,uint32_t *completed,fg_error *err);
