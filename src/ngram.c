...
fg_status fg_q38_ngram_lookup_range(const int32_t *tokens,size_t history_count,
    size_t first,uint32_t count,uint64_t *rows,uint64_t *addresses,fg_error *err){
    if(!tokens||!count||!rows||!addresses||first>history_count||
       count>history_count-first||count>FG_NGRAM_PREFILL_MAX_TOKENS){
        fg_error_set(err,FG_ERR_ARGUMENT,"invalid n-gram suffix range");
        return FG_ERR_ARGUMENT;
    }
    size_t begin=first>2u?first-2u:0u,end=first+count;
    for(size_t i=begin;i<end;i++)if((uint32_t)tokens[i]>=FG_Q38_VOCAB_SIZE){
        fg_error_set(err,FG_ERR_FORMAT,"token %zu is outside Qwen3.8 vocabulary",i);
        return FG_ERR_FORMAT;
    }
    size_t segment_start=begin;
    for(size_t i=begin;i<first;i++)
        if((uint32_t)tokens[i]==FG_Q38_EOS_TOKEN)segment_start=i+1u;
    for(uint32_t i=0;i<count;i++){
        size_t pos=first+i;
        q38_ngram_fill(tokens,pos+1u,segment_start,
            rows+(uint64_t)i*FG_NGRAM_HEAD_COUNT,
            addresses+(uint64_t)i*FG_NGRAM_HEAD_COUNT);
...
fg_status fg_ngram_store_lookup_prefill(fg_ngram_store *s,const int32_t *tokens,size_t history_count,uint32_t first_token,uint32_t token_count,fg_vk_tensor **embedding,fg_error *err){
    ...
    uint64_t addresses[FG_NGRAM_PREFILL_MAX_ROWS],rows[FG_NGRAM_PREFILL_MAX_ROWS];
    fg_status status=fg_q38_ngram_lookup_range(tokens,history_count,first_token,
