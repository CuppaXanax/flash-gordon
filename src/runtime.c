...
static fg_status runtime_reset_state(fg_runtime *runtime,fg_prefix_reset_reason reason,
    ...
    if(status!=FG_OK)return status;
    if(!runtime->qsa_open){
        if(unlink(runtime->qsa_path)!=0&&errno!=ENOENT){
            fg_error_set(err,FG_ERR_IO,"remove stale QSA session: %s",strerror(errno));
            return FG_ERR_IO;
        }
...
fg_status fg_eval_main(const char *path,const char *prompt,uint32_t generate,fg_error *err){
    ...
    if(status==FG_OK)status=coordinator_open(&coordinator,manifest,directory,err);
