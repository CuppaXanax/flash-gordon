...
static fg_status begin_session(fg_fabric *fabric,uint32_t peer,const fg_frame_header *header,uint64_t *session_id,fg_error *err){uint64_t request=fg_frame_request_id(header);if(peer!=0u||!request||*session_id){fg_error_set(err,FG_ERR_MISMATCH,"invalid or duplicate session begin");return FG_ERR_MISMATCH;}*session_id=request;return fg_fabric_send(fabric,peer,FG_FABRIC_CONTROL,FG_MSG_SESSION_READY,request,0,0,NULL,0,err);}

static fg_status handle_ngram_work(fg_fabric *fabric,const fg_ngram_resident *resident,uint32_t self,uint64_t session_id,uint32_t peer,const fg_frame_header *header,const uint8_t *payload,uint32_t bytes,fg_error *err){fg_ngram_work work;fg_status status=fg_ngram_work_decode(&work,payload,bytes,err);uint32_t head_begin=0,head_count=0;uint64_t request=fg_frame_request_id(header);if(status==FG_OK)status=fg_q38_ngram_rank_heads(self,&head_begin,&head_count,err);if(status==FG_OK&&(!session_id||request!=session_id||peer!=0u||work.source_rank!=0u||work.destination_rank!=self||work.head_begin!=head_begin||work.head_count!=head_count||fg_frame_sequence(header)!=work.token_index)){fg_error_set(err,FG_ERR_MISMATCH,"stale or misrouted resident n-gram work");status=FG_ERR_MISMATCH;}fg_ngram_result result={.source_rank=(uint8_t)self,.destination_rank=0u,.head_begin=(uint8_t)head_begin,.head_count=(uint8_t)head_count,.token_index=work.token_index};if(status==FG_OK)status=fg_ngram_resident_read(resident,work.rows,work.head_count,result.packed,sizeof(result.packed),err);uint8_t wire[FG_NGRAM_RESULT_MAX_BYTES];uint32_t result_bytes=0;if(status==FG_OK)status=fg_ngram_result_encode(wire,sizeof(wire),&result_bytes,&result,err);if(status==FG_OK)status=fg_fabric_send(fabric,0u,FG_FABRIC_BULK,FG_MSG_NGRAM_RESULT,request,work.token_index,0,wire,result_bytes,err);return status;}
...
static fg_status rank_worker_loop(fg_fabric *fabric,fg_expert_executor *expert,fg_output_executor *output,const fg_ngram_resident *ngram,fg_model *model,const fg_manifest *manifest,uint32_t self,fg_error *err){
    ...
    while(status==FG_OK){uint32_t peer=0,bytes=0;fg_frame_header header;fg_fabric_class ready_class;fg_fabric_recv_timing receive_timing={0};receive_timing.poll_start_ns=critical_ns();status=fg_fabric_wait_ready(fabric,3u,&peer,&ready_class,err);receive_timing.ready_ns=critical_ns();if(status!=FG_OK)break;if(ready_class==FG_FABRIC_BULK){status=fg_fabric_recv_timed(fabric,peer,FG_FABRIC_BULK,&header,prefill.receive,prefill.receive_capacity,&bytes,&receive_timing,err);fg_message_type type=status==FG_OK?fg_frame_type(&header):0;if(status==FG_OK&&type==FG_MSG_PREFILL_WORK)status=handle_prefill_expert_work(fabric,expert,manifest,self,session_id,peer,&header,prefill.receive,bytes,&prefill,err);else if(status==FG_OK){fg_error_set(err,FG_ERR_FORMAT,"rank %u received unsupported bulk message %u",self,type);status=FG_ERR_FORMAT;}continue;}status=fg_fabric_recv_timed(fabric,peer,FG_FABRIC_CONTROL,&header,control,control_capacity,&bytes,&receive_timing,err);if(status!=FG_OK)break;fg_message_type type=fg_frame_type(&header);if(type==FG_MSG_DECODE_WORK){if(!session_id||fg_frame_request_id(&header)!=session_id){fg_error_set(err,FG_ERR_MISMATCH,"stale expert work request");status=FG_ERR_MISMATCH;}else status=handle_expert_work(fabric,expert,fg_model_vk(model),self,peer,&header,control,bytes,&receive_timing,ew_result,ew_wire,err);}else if(type==FG_MSG_NGRAM_WORK)status=handle_ngram_work(fabric,ngram,self,session_id,peer,&header,control,bytes,err);else if(type==FG_MSG_SESSION_BEGIN){if(bytes){fg_error_set(err,FG_ERR_FORMAT,"session begin payload must be empty");status=FG_ERR_FORMAT;}else status=begin_session(fabric,peer,&header,&session_id,err);}else if(type==FG_MSG_OUTPUT_WORK)status=handle_output_work(fabric,output,fg_model_vk(model),self,session_id,peer,&header,control,bytes,hyper,err);else{fg_error_set(err,FG_ERR_FORMAT,"rank %u received unsupported control message %u",self,type);status=FG_ERR_FORMAT;}}
    fg_vk_tensor_destroy(hyper);prefill_worker_buffers_destroy(&prefill);free(ew_wire);free(ew_result);free(control);return status;
}

fg_status fg_rank_main(const char *path,uint32_t rank,fg_error *err){if(rank>=FG_RANK_COUNT){fg_error_set(err,FG_ERR_ARGUMENT,"rank must be 0..7");return FG_ERR_ARGUMENT;}fg_manifest *manifest=NULL;fg_status status=load_checked(path,&manifest,err);char directory[1024];if(status==FG_OK)status=manifest_directory(path,directory,err);fg_model *model=NULL;fg_expert_executor *expert=NULL;fg_output_executor *output=NULL;fg_ngram_resident *ngram=NULL;fg_fabric *fabric=NULL;if(status==FG_OK)status=fg_model_open(&model,manifest,directory,rank,err);if(status==FG_OK)status=fg_expert_executor_create(&expert,model,err);if(status==FG_OK&&rank==4u)status=fg_output_executor_create(&output,model,err);uint32_t head_begin=0,head_count=0;uint64_t row_begin=0,row_count=0;char ngram_path[1200];if(status==FG_OK)status=fg_q38_ngram_rank_heads(rank,&head_begin,&head_count,err);if(status==FG_OK)status=fg_q38_ngram_head_range(head_begin,head_count,&row_begin,&row_count,err);if(status==FG_OK&&snprintf(ngram_path,sizeof(ngram_path),"%s/ngram-rank-%02u.iq4nl",directory,rank)>=(int)sizeof(ngram_path)){fg_error_set(err,FG_ERR_LIMIT,"resident n-gram shard path is too long");status=FG_ERR_LIMIT;}if(status==FG_OK)status=fg_ngram_resident_open(&ngram,ngram_path,row_begin,row_count,err);if(status==FG_OK)status=fg_fabric_open(&fabric,manifest,rank,err);if(status==FG_OK)status=rank_ready(fabric,rank,err);if(status==FG_OK){printf("rank %u READY: %.3f GiB sealed weights, heads %u-%u resident on %s\n",rank,(double)fg_model_weight_bytes(model)/(1024.0*1024.0*1024.0),head_begin,head_begin+head_count-1u,fg_vk_device_name(fg_model_vk(model)));fflush(stdout);status=rank_worker_loop(fabric,expert,output,ngram,model,manifest,rank,err);}fg_fabric_close(fabric);fg_ngram_resident_close(ngram);fg_output_executor_destroy(output);fg_expert_executor_destroy(expert);fg_model_close(model);free(manifest);return status;}
...
static fg_status coordinator_ngram_resident(fg_coordinator *coordinator,const int32_t *history,size_t history_count,uint32_t token_index,fg_vk_tensor **embedding,fg_error *err){uint64_t rows[FG_NGRAM_HEAD_COUNT],addresses[FG_NGRAM_HEAD_COUNT];fg_status status=fg_q38_ngram_lookup(history,history_count,rows,addresses,err);uint8_t work_wire[FG_NGRAM_WORK_MAX_BYTES],result_wire[FG_NGRAM_RESULT_MAX_BYTES],packed[FG_NGRAM_HEAD_COUNT*FG_NGRAM_ROW_BYTES];double start=dispatch_ts();for(uint32_t rank=1u;status==FG_OK&&rank<FG_RANK_COUNT;rank++){uint32_t head_begin=0,head_count=0;status=fg_q38_ngram_rank_heads(rank,&head_begin,&head_count,err);fg_ngram_work work={.source_rank=0u,.destination_rank=(uint8_t)rank,.head_begin=(uint8_t)head_begin,.head_count=(uint8_t)head_count,.token_index=token_index};if(status==FG_OK)memcpy(work.rows,rows+head_begin,(size_t)head_count*sizeof(*rows));uint32_t bytes=0;if(status==FG_OK)status=fg_ngram_work_encode(work_wire,sizeof(work_wire),&bytes,&work,err);if(status==FG_OK)status=fg_fabric_send(coordinator->fabric,rank,FG_FABRIC_CONTROL,FG_MSG_NGRAM_WORK,coordinator->session_id,token_index,0,work_wire,bytes,err);}double fired=dispatch_ts();bool received[FG_RANK_COUNT]={0};for(uint32_t count=1u;status==FG_OK&&count<FG_RANK_COUNT;count++){uint32_t peer=0,bytes=0;fg_frame_header header;status=fg_fabric_recv_any(coordinator->fabric,FG_FABRIC_BULK,&peer,&header,result_wire,sizeof(result_wire),&bytes,err);if(status==FG_OK&&(peer==0u||peer>=FG_RANK_COUNT||received[peer]||fg_frame_type(&header)!=FG_MSG_NGRAM_RESULT||fg_frame_request_id(&header)!=coordinator->session_id||fg_frame_sequence(&header)!=token_index)){fg_error_set(err,FG_ERR_MISMATCH,"stale or duplicate resident n-gram result from rank %u",peer);status=FG_ERR_MISMATCH;}fg_ngram_result result;if(status==FG_OK)status=fg_ngram_result_decode(&result,result_wire,bytes,err);uint32_t head_begin=0,head_count=0;if(status==FG_OK)status=fg_q38_ngram_rank_heads(peer,&head_begin,&head_count,err);if(status==FG_OK&&(result.source_rank!=peer||result.destination_rank!=0u||result.token_index!=token_index||result.head_begin!=head_begin||result.head_count!=head_count)){fg_error_set(err,FG_ERR_MISMATCH,"misrouted resident n-gram result from rank %u",peer);status=FG_ERR_MISMATCH;}if(status==FG_OK){memcpy(packed+(uint64_t)head_begin*FG_NGRAM_ROW_BYTES,result.packed,(size_t)head_count*FG_NGRAM_ROW_BYTES);received[peer]=true;}}double collected=dispatch_ts();if(status==FG_OK)status=fg_ngram_store_decode_packed(coordinator->ngram,packed,FG_NGRAM_HEAD_COUNT,embedding,err);double decoded=dispatch_ts();if(frame_trace_enabled())fprintf(stderr,"NGRAM_RESIDENT_TRACE token=%u fire_ms=%.3f collect_ms=%.3f dequant_ms=%.3f total_ms=%.3f\n",token_index,fired-start,collected-fired,decoded-collected,decoded-start);(void)addresses;return status;}
