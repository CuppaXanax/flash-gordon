...
void fg_expert_executor_destroy(fg_expert_executor *executor){if(!executor)return;for(uint32_t layer=0;layer<FG_LAYER_COUNT;layer++)fg_vk_expert_graph_destroy(executor->decode_graph[layer]);fg_vk_tensor_destroy(executor->reduced);fg_vk_tensor_destroy(executor->down);fg_vk_tensor_destroy(executor->mid);fg_vk_tensor_destroy(executor->up);fg_vk_tensor_destroy(executor->gate);fg_vk_tensor_destroy(executor->gates);fg_vk_tensor_destroy(executor->tiles);fg_vk_tensor_destroy(executor->activation);free(executor);}
...
static fg_status project_down(fg_vk_context *vk,fg_vk_tensor *output,const fg_vk_tensor *weights,const fg_vk_tensor *tiles,const fg_vk_tensor *input,const fg_tensor_record *record,uint32_t routed_pairs,uint32_t tile_count,fg_error *err){uint32_t stride=(uint32_t)(record->bytes/FG_EXPERTS_PER_RANK);if(fg_vk_tensor_get_format(weights)==FG_VK_TENSOR_FORMAT_Q5_1_EXPERT_COOKED)return fg_vk_moe_q5_1_down_cooked_pairs(vk,output,weights,tiles,input,FG_HIDDEN_SIZE,640u,stride,routed_pairs,false,tile_count,err);if(record->ggml_type==7u)return fg_vk_moe_q5_1_down(vk,output,weights,tiles,input,FG_HIDDEN_SIZE,640u,stride,FG_TOP_K,false,tile_count,err);if(record->ggml_type==8u)return fg_vk_moe_q8_0_down(vk,output,weights,tiles,input,FG_HIDDEN_SIZE,640u,stride,FG_TOP_K,false,tile_count,err);fg_error_set(err,FG_ERR_FORMAT,"unsupported expert down quant %u",record->ggml_type);return FG_ERR_FORMAT;}

static fg_status create_decode_graphs(fg_expert_executor *executor,fg_error *err){
    uint32_t rank=fg_model_rank(executor->model);fg_vk_context *vk=fg_model_vk(executor->model);
    for(uint32_t layer=0;layer<FG_LAYER_COUNT;layer++){char gate_name[FG_TENSOR_NAME_MAX],up_name[FG_TENSOR_NAME_MAX],down_name[FG_TENSOR_NAME_MAX];if(!tensor_name(gate_name,layer,"ffn_gate_exps",rank)||!tensor_name(up_name,layer,"ffn_up_exps",rank)||!tensor_name(down_name,layer,"ffn_down_exps",rank)){fg_error_set(err,FG_ERR_LIMIT,"expert graph tensor name overflow");return FG_ERR_LIMIT;}fg_vk_tensor *gate_weight=fg_model_tensor(executor->model,gate_name),*up_weight=fg_model_tensor(executor->model,up_name),*down_weight=fg_model_tensor(executor->model,down_name);const fg_tensor_record *gate_record=fg_model_tensor_record(executor->model,gate_name),*up_record=fg_model_tensor_record(executor->model,up_name),*down_record=fg_model_tensor_record(executor->model,down_name);if(!gate_weight&&!up_weight&&!down_weight&&!gate_record&&!up_record&&!down_record)continue;if(!gate_weight||!up_weight||!down_weight||!gate_record||!up_record||!down_record||gate_record->bytes%FG_EXPERTS_PER_RANK||up_record->bytes%FG_EXPERTS_PER_RANK||down_record->bytes%FG_EXPERTS_PER_RANK||gate_record->bytes/FG_EXPERTS_PER_RANK>UINT32_MAX||up_record->bytes/FG_EXPERTS_PER_RANK>UINT32_MAX||down_record->bytes/FG_EXPERTS_PER_RANK>UINT32_MAX){fg_error_set(err,FG_ERR_MISMATCH,"rank is missing or has malformed layer %u expert graph tensors",layer);return FG_ERR_MISMATCH;}fg_status status=fg_vk_expert_graph_create(vk,&executor->decode_graph[layer],executor->activation,executor->tiles,executor->gates,executor->gate,executor->up,executor->mid,executor->down,executor->reduced,gate_weight,up_weight,down_weight,gate_record->ggml_type,up_record->ggml_type,down_record->ggml_type,FG_HIDDEN_SIZE,640u,(uint32_t)(gate_record->bytes/FG_EXPERTS_PER_RANK),(uint32_t)(up_record->bytes/FG_EXPERTS_PER_RANK),(uint32_t)(down_record->bytes/FG_EXPERTS_PER_RANK),FG_EXPERTS_PER_RANK,FG_TOP_K,err);if(status!=FG_OK)return status;}
    return FG_OK;
}
...
fg_status fg_expert_decode(fg_expert_executor *executor,const fg_decode_work *work,fg_expert_result *result,fg_error *err){
    ...
    fg_vk_context *vk=fg_model_vk(executor->model);fg_status status=fg_vk_tensor_write(executor->activation,0,work->activation_q8k,FG_Q8K_ACTIVATION_BYTES,err);if(status==FG_OK)status=fg_vk_tensor_write(executor->tiles,0,schedule,sizeof(schedule),err);if(status==FG_OK)status=fg_vk_tensor_write(executor->gates,0,work->gates,(uint64_t)work->selected_count*4u,err);
    if(status==FG_OK&&!fg_vk_profile_active(vk)){if(!executor->decode_graph[work->layer]){fg_error_set(err,FG_ERR_MISMATCH,"rank has no fixed expert graph for layer %u",work->layer);status=FG_ERR_MISMATCH;}else status=fg_vk_expert_graph_execute(executor->decode_graph[work->layer],err);}else if(status==FG_OK){char gate_name[FG_TENSOR_NAME_MAX],up_name[FG_TENSOR_NAME_MAX],down_name[FG_TENSOR_NAME_MAX];if(!tensor_name(gate_name,work->layer,"ffn_gate_exps",rank)||!tensor_name(up_name,work->layer,"ffn_up_exps",rank)||!tensor_name(down_name,work->layer,"ffn_down_exps",rank)){fg_error_set(err,FG_ERR_LIMIT,"expert tensor name overflow");status=FG_ERR_LIMIT;}fg_vk_tensor *gate_weight=status==FG_OK?fg_model_tensor(executor->model,gate_name):NULL,*up_weight=status==FG_OK?fg_model_tensor(executor->model,up_name):NULL,*down_weight=status==FG_OK?fg_model_tensor(executor->model,down_name):NULL;const fg_tensor_record *gate_record=status==FG_OK?fg_model_tensor_record(executor->model,gate_name):NULL,*up_record=status==FG_OK?fg_model_tensor_record(executor->model,up_name):NULL,*down_record=status==FG_OK?fg_model_tensor_record(executor->model,down_name):NULL;if(status==FG_OK&&(!gate_weight||!up_weight||!down_weight||!gate_record||!up_record||!down_record)){fg_error_set(err,FG_ERR_MISMATCH,"rank is missing layer %u expert tensors",work->layer);status=FG_ERR_MISMATCH;}if(status==FG_OK)status=fg_vk_begin(vk,err);uint32_t tiles=work->selected_count;if(status==FG_OK)status=fg_vk_profile_set_scope(vk,"expert_gate",err);if(status==FG_OK)status=fg_vk_moe_kquant(vk,executor->gate,gate_weight,executor->activation,executor->tiles,gate_record->ggml_type,640u,FG_HIDDEN_SIZE,(uint32_t)(gate_record->bytes/FG_EXPERTS_PER_RANK),FG_TOP_K,FG_TOP_K,false,tiles,err);if(status==FG_OK)status=fg_vk_profile_set_scope(vk,"expert_up",err);if(status==FG_OK)status=fg_vk_moe_kquant(vk,executor->up,up_weight,executor->activation,executor->tiles,up_record->ggml_type,640u,FG_HIDDEN_SIZE,(uint32_t)(up_record->bytes/FG_EXPERTS_PER_RANK),FG_TOP_K,FG_TOP_K,false,tiles,err);if(status==FG_OK)status=fg_vk_profile_set_scope(vk,"expert_activation",err);if(status==FG_OK)status=fg_vk_swiglu(vk,executor->mid,executor->gate,executor->up,FG_EXPERT_MID_VALUES,err);uint32_t down_stride=status==FG_OK?(uint32_t)(down_record->bytes/FG_EXPERTS_PER_RANK):0u;if(status==FG_OK)status=fg_vk_profile_set_scope(vk,"expert_down",err);if(status==FG_OK&&down_record->ggml_type==7u)status=fg_vk_moe_q5_1_down(vk,executor->down,down_weight,executor->tiles,executor->mid,FG_HIDDEN_SIZE,640u,down_stride,FG_TOP_K,false,tiles,err);else if(status==FG_OK&&down_record->ggml_type==8u)status=fg_vk_moe_q8_0_down(vk,executor->down,down_weight,executor->tiles,executor->mid,FG_HIDDEN_SIZE,640u,down_stride,FG_TOP_K,false,tiles,err);else if(status==FG_OK){fg_error_set(err,FG_ERR_FORMAT,"unsupported expert down quant %u",down_record->ggml_type);status=FG_ERR_FORMAT;}if(status==FG_OK)status=fg_vk_profile_set_scope(vk,"expert_reduce",err);if(status==FG_OK)status=fg_vk_moe_reduce(vk,executor->reduced,executor->down,executor->gates,executor->tiles,FG_HIDDEN_SIZE,work->selected_count,FG_TOP_K,err);if(status==FG_OK){fg_status end_status=fg_vk_end(vk,err);if(end_status!=FG_OK)status=end_status;}else if(fg_vk_batch_active(vk))fg_vk_end(vk,err);}
...
fg_status fg_expert_prefill(fg_expert_executor *executor,const fg_prefill_work *work,fg_prefill_result *result,fg_prefill_result_pair *pair_storage,uint32_t pair_capacity,float *output_storage,uint64_t output_capacity_values,fg_error *err){
    ...
    bool cooked=status==FG_OK&&fg_vk_tensor_get_format(gate_weight)==FG_VK_TENSOR_FORMAT_K_QUANT_EXPERT_COOKED&&fg_vk_tensor_get_format(up_weight)==FG_VK_TENSOR_FORMAT_K_QUANT_EXPERT_COOKED&&fg_vk_tensor_get_format(down_weight)==FG_VK_TENSOR_FORMAT_Q5_1_EXPERT_COOKED;if(cooked){uint32_t *flat=malloc((size_t)work->pair_count*9u*4u);if(!flat){fg_error_set(err,FG_ERR_OOM,"allocate cooked expert prefill schedule");status=FG_ERR_OOM;}else{for(uint32_t i=0;i<work->pair_count*9u;i++)flat[i]=UINT32_MAX;for(uint32_t i=0;i<work->pair_count;i++){flat[i*9u]=locals[i];flat[i*9u+1u]=work->pairs[i].token_slot*FG_TOP_K+work->pairs[i].routing_slot;}free(schedule);schedule=flat;tile_count=work->pair_count;}}
