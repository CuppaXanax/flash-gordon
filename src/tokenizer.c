...
fg_status fg_tokenizer_encode(const fg_tokenizer *tokenizer,const char *text,bool allow_special,fg_tokens *tokens,fg_error *err){if(!tokenizer||!text||!tokens){fg_error_set(err,FG_ERR_ARGUMENT,"invalid tokenizer encode arguments");return FG_ERR_ARGUMENT;}size_t bytes=strlen(text),offset=0;if(tokenizer->add_bos&&tokens->count==0){fg_status status=tokens_push(tokens,tokenizer->bos,err);if(status!=FG_OK)return status;}while(offset<bytes){size_t end=next_piece(text,bytes,offset);if(end<=offset||end>bytes){fg_error_set(err,FG_ERR_FORMAT,"Qwen pre-tokenizer made no progress");return FG_ERR_FORMAT;}if(allow_special){size_t scan=offset,special_offset=SIZE_MAX,special_length=0;uint32_t special=0;while(scan<end){uint32_t candidate;size_t candidate_length;if(special_at(tokenizer,text+scan,bytes-scan,&candidate,&candidate_length)){special_offset=scan;special=candidate;special_length=candidate_length;break;}scan++;}if(special_offset!=SIZE_MAX){if(special_offset>offset){fg_status status=encode_piece(tokenizer,text+offset,special_offset-offset,tokens,err);if(status!=FG_OK)return status;}fg_status status=tokens_push(tokens,special,err);if(status!=FG_OK)return status;offset=special_offset+special_length;continue;}}fg_status status=encode_piece(tokenizer,text+offset,end-offset,tokens,err);if(status!=FG_OK)return status;offset=end;}return FG_OK;}
...
fg_status fg_tokenizer_validate_qwen38(const fg_tokenizer *tokenizer,fg_error *err){
    ...
    static const uint32_t glued_ids[]={14556,93,248046,198};
    static const struct{const char *text;bool special;const uint32_t *ids;size_t count;}vectors[]={
        ...
        {"hello~<|im_end|>\n",true,glued_ids,sizeof(glued_ids)/sizeof(glued_ids[0])}
    };
