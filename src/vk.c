...
fg_status fg_vk_open(fg_vk_context **out,fg_error *err){
    ...
    c->moe_q5_1=(fg_vk_kernel){.file="fg_moe_q5_1_down.spv",.bindings=4,.push_bytes=28};c->moe_q8_0=(fg_vk_kernel){.file="fg_moe_q8_0_down.spv",.bindings=4,.push_bytes=28};c->moe_reduce=(fg_vk_kernel){.file="fg_moe_reduce.spv",.bindings=4,.push_bytes=12};c->kquant=(fg_vk_kernel){.file="fg_moe_kquant.spv",.bindings=4,.push_bytes=36};
...
fg_status fg_vk_expert_graph_create(fg_vk_context *c,fg_vk_expert_graph **out,fg_vk_tensor *activation,fg_vk_tensor *tiles,fg_vk_tensor *gates,fg_vk_tensor *gate,fg_vk_tensor *up,fg_vk_tensor *mid,fg_vk_tensor *down,fg_vk_tensor *reduced,const fg_vk_tensor *gate_weights,const fg_vk_tensor *up_weights,const fg_vk_tensor *down_weights,uint32_t gate_type,uint32_t up_type,uint32_t down_type,uint32_t hidden_width,uint32_t mid_width,uint32_t gate_expert_stride,uint32_t up_expert_stride,uint32_t down_expert_stride,uint32_t weight_experts,uint32_t slots,fg_error *err){
    ...
    struct{uint32_t out_dim,blocks,row_bytes,expert_stride,type,n_used,routed_pairs,packed_weights,reserved;}gate_push={mid_width,hidden_width/256u,(uint32_t)gate_row,gate_expert_stride,gate_type,slots,slots,0u,0u},up_push={mid_width,hidden_width/256u,(uint32_t)up_row,up_expert_stride,up_type,slots,slots,0u,0u};struct{uint32_t values;}swiglu_push={slots*mid_width};struct{uint32_t out_dim,in_dim,row_bytes,expert_stride,n_used,packed_weights,reserved;}down_push={hidden_width,mid_width,(uint32_t)down_row,down_expert_stride,slots,0u,0u};struct{uint32_t width,selected_count,slot_count;}reduce_push={hidden_width,slots,slots};
    const fg_vk_tensor *gate_bindings[]={gate_weights,activation,tiles,gate},*up_bindings[]={up_weights,activation,tiles,up},*swiglu_bindings[]={gate,up,mid},*down_bindings[]={down_weights,tiles,mid,down},*reduce_bindings[]={down,gates,tiles,reduced};fg_vk_kernel *down_kernel=down_type==7u?&c->moe_q5_1:&c->moe_q8_0;fg_status status=expert_graph_dispatch(graph,&c->kquant,gate_bindings,&gate_push,(mid_width+7u)/8u,slots,1u,false,err);if(status==FG_OK)status=expert_graph_dispatch(graph,&c->kquant,up_bindings,&up_push,(mid_width+7u)/8u,slots,1u,false,err);if(status==FG_OK)status=expert_graph_dispatch(graph,&c->swiglu,swiglu_bindings,&swiglu_push,(slots*mid_width+255u)/256u,1u,1u,true,err);if(status==FG_OK)status=expert_graph_dispatch(graph,down_kernel,down_bindings,&down_push,(hidden_width+7u)/8u,slots,1u,true,err);if(status==FG_OK)status=expert_graph_dispatch(graph,&c->moe_reduce,reduce_bindings,&reduce_push,(hidden_width+255u)/256u,1u,1u,true,err);
