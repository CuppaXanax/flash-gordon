...
typedef struct fg_vk_descriptor_cache_entry {bool valid;fg_vk_kernel *kernel;VkDescriptorSet set;fg_vk_allocation *allocations[16];uint64_t offsets[16],bytes[16];} fg_vk_descriptor_cache_entry;

#define FG_VK_PROFILE_MAX_DISPATCHES 256u
#define FG_VK_PROFILE_QUERY_COUNT (2u+2u*FG_VK_PROFILE_MAX_DISPATCHES)
#define FG_VK_DESCRIPTOR_CACHE_CAPACITY 4096u

struct fg_vk_context {
    VkInstance instance;VkPhysicalDevice physical;VkDevice device;VkQueue queue;uint32_t queue_family;
    VkCommandPool command_pool;VkCommandBuffer command;VkFence fence;VkDescriptorPool descriptor_pool,runtime_descriptor_pool;VkPipelineCache pipeline_cache;
    ...
    fg_vk_descriptor_cache_entry *descriptor_cache;
...
static uint64_t descriptor_hash(const fg_vk_kernel *kernel,const fg_vk_tensor *const *tensors){uint64_t hash=(uint64_t)(uintptr_t)kernel*UINT64_C(0x9e3779b97f4a7c15);for(uint32_t i=0;i<kernel->bindings;i++){hash^=(uint64_t)(uintptr_t)tensors[i]->allocation+UINT64_C(0x9e3779b97f4a7c15)+(hash<<6u)+(hash>>2u);hash^=tensors[i]->offset+UINT64_C(0x9e3779b97f4a7c15)+(hash<<6u)+(hash>>2u);hash^=tensors[i]->bytes+UINT64_C(0x9e3779b97f4a7c15)+(hash<<6u)+(hash>>2u);}return hash;}

static bool descriptor_matches(const fg_vk_descriptor_cache_entry *entry,const fg_vk_kernel *kernel,const fg_vk_tensor *const *tensors){if(!entry->valid||entry->kernel!=kernel)return false;for(uint32_t i=0;i<kernel->bindings;i++)if(entry->allocations[i]!=tensors[i]->allocation||entry->offsets[i]!=tensors[i]->offset||entry->bytes[i]!=tensors[i]->bytes)return false;return true;}

static void descriptor_update(fg_vk_context *c,fg_vk_kernel *kernel,const fg_vk_tensor *const *tensors,VkDescriptorSet set){VkDescriptorBufferInfo info[16];VkWriteDescriptorSet write[16];for(uint32_t i=0;i<kernel->bindings;i++){info[i]=(VkDescriptorBufferInfo){.buffer=tensors[i]->allocation->buffer,.offset=tensors[i]->offset,.range=tensors[i]->bytes};write[i]=(VkWriteDescriptorSet){.sType=VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,.dstSet=set,.dstBinding=i,.descriptorCount=1,.descriptorType=VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,.pBufferInfo=&info[i]};}vkUpdateDescriptorSets(c->device,kernel->bindings,write,0,NULL);if(c->profile_active)c->profile.descriptor_updates++;}

static fg_status descriptor_get(fg_vk_context *c,fg_vk_kernel *kernel,const fg_vk_tensor *const *tensors,VkDescriptorSet *set,bool *cached,fg_error *err){
    *cached=false;fg_vk_descriptor_cache_entry *entry=NULL;if(c->descriptor_pool==c->runtime_descriptor_pool&&c->descriptor_cache)entry=&c->descriptor_cache[descriptor_hash(kernel,tensors)&(FG_VK_DESCRIPTOR_CACHE_CAPACITY-1u)];
    if(entry&&descriptor_matches(entry,kernel,tensors)){*set=entry->set;*cached=true;if(c->profile_active)c->profile.descriptor_cache_hits++;return FG_OK;}
    if(entry&&!entry->valid){if(entry->set&&entry->kernel!=kernel){vkFreeDescriptorSets(c->device,c->runtime_descriptor_pool,1,&entry->set);entry->set=VK_NULL_HANDLE;}if(!entry->set){VkDescriptorSetAllocateInfo allocate={.sType=VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO,.descriptorPool=c->runtime_descriptor_pool,.descriptorSetCount=1,.pSetLayouts=&kernel->set_layout};VkResult result=vkAllocateDescriptorSets(c->device,&allocate,&entry->set);if(result!=VK_SUCCESS)return vk_error(err,"allocate cached descriptor set",result);}descriptor_update(c,kernel,tensors,entry->set);entry->valid=true;entry->kernel=kernel;for(uint32_t i=0;i<kernel->bindings;i++){entry->allocations[i]=tensors[i]->allocation;entry->offsets[i]=tensors[i]->offset;entry->bytes[i]=tensors[i]->bytes;}*set=entry->set;*cached=true;return FG_OK;}
    VkDescriptorSetAllocateInfo allocate={.sType=VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO,.descriptorPool=c->descriptor_pool,.descriptorSetCount=1,.pSetLayouts=&kernel->set_layout};VkResult result=vkAllocateDescriptorSets(c->device,&allocate,set);if(result!=VK_SUCCESS)return vk_error(err,"allocate descriptor set",result);descriptor_update(c,kernel,tensors,*set);return FG_OK;
}

static void descriptor_invalidate(fg_vk_context *c,const fg_vk_allocation *allocation){if(!c->descriptor_cache)return;for(uint32_t slot=0;slot<FG_VK_DESCRIPTOR_CACHE_CAPACITY;slot++){fg_vk_descriptor_cache_entry *entry=&c->descriptor_cache[slot];if(!entry->valid)continue;for(uint32_t i=0;i<entry->kernel->bindings;i++)if(entry->allocations[i]==allocation){entry->valid=false;break;}}}
...
fg_status fg_vk_open(fg_vk_context **out,fg_error *err){
    ...
    VkFenceCreateInfo fc={.sType=VK_STRUCTURE_TYPE_FENCE_CREATE_INFO};if((vr=vkCreateFence(c->device,&fc,NULL,&c->fence))!=VK_SUCCESS){fg_vk_close(c);return vk_error(err,"create fence",vr);}VkDescriptorPoolSize ps={.type=VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,.descriptorCount=FG_VK_DESCRIPTOR_CACHE_CAPACITY*12u};VkDescriptorPoolCreateInfo dp={.sType=VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO,.flags=VK_DESCRIPTOR_POOL_CREATE_FREE_DESCRIPTOR_SET_BIT,.maxSets=FG_VK_DESCRIPTOR_CACHE_CAPACITY+256u,.poolSizeCount=1,.pPoolSizes=&ps};if((vr=vkCreateDescriptorPool(c->device,&dp,NULL,&c->descriptor_pool))!=VK_SUCCESS){fg_vk_close(c);return vk_error(err,"create descriptor pool",vr);}c->runtime_descriptor_pool=c->descriptor_pool;c->descriptor_cache=calloc(FG_VK_DESCRIPTOR_CACHE_CAPACITY,sizeof(*c->descriptor_cache));if(!c->descriptor_cache){fg_vk_close(c);fg_error_set(err,FG_ERR_OOM,"allocate descriptor cache");return FG_ERR_OOM;}VkPipelineCacheCreateInfo pci={.sType=VK_STRUCTURE_TYPE_PIPELINE_CACHE_CREATE_INFO};if((vr=vkCreatePipelineCache(c->device,&pci,NULL,&c->pipeline_cache))!=VK_SUCCESS){fg_vk_close(c);return vk_error(err,"create pipeline cache",vr);}
...
void fg_vk_tensor_destroy(fg_vk_tensor *t){if(!t)return;fg_vk_allocation *a=t->allocation;if(--a->references==0){descriptor_invalidate(t->context,a);vkUnmapMemory(t->context->device,a->memory);vkFreeMemory(t->context->device,a->memory,NULL);vkDestroyBuffer(t->context->device,a->buffer,NULL);free(a);}free(t);}
...
static fg_status dispatch(fg_vk_context *c,fg_vk_kernel *kernel,const fg_vk_tensor *const *tensors,const void *push,uint32_t gx,uint32_t gy,uint32_t gz,fg_error *err){
    fg_status status=create_kernel(c,kernel,err);if(status!=FG_OK)return status;VkDescriptorSet set;bool cached=false;status=descriptor_get(c,kernel,tensors,&set,&cached,err);if(status!=FG_OK)return status;VkResult vr;
    if(c->batch_depth){VkMemoryBarrier bar={.sType=VK_STRUCTURE_TYPE_MEMORY_BARRIER,.srcAccessMask=VK_ACCESS_SHADER_WRITE_BIT,.dstAccessMask=VK_ACCESS_SHADER_READ_BIT|VK_ACCESS_SHADER_WRITE_BIT};vkCmdPipelineBarrier(c->command,VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,0,1,&bar,0,NULL,0,NULL);vkCmdBindPipeline(c->command,VK_PIPELINE_BIND_POINT_COMPUTE,kernel->pipeline);vkCmdBindDescriptorSets(c->command,VK_PIPELINE_BIND_POINT_COMPUTE,kernel->layout,0,1,&set,0,NULL);if(kernel->push_bytes)vkCmdPushConstants(c->command,kernel->layout,VK_SHADER_STAGE_COMPUTE_BIT,0,kernel->push_bytes,push);bool profiled=profile_dispatch_begin(c,kernel);vkCmdDispatch(c->command,gx,gy,gz);profile_dispatch_end(c,profiled);if(!cached&&c->batch_set_count<256)c->batch_sets[c->batch_set_count++]=set;return FG_OK;}
    vkResetFences(c->device,1,&c->fence);vkResetCommandBuffer(c->command,0);VkCommandBufferBeginInfo begin={.sType=VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO,.flags=VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT};if((vr=vkBeginCommandBuffer(c->command,&begin))!=VK_SUCCESS)return vk_error(err,"begin compute command",vr);profile_command_begin(c);VkMemoryBarrier before={.sType=VK_STRUCTURE_TYPE_MEMORY_BARRIER,.srcAccessMask=VK_ACCESS_HOST_WRITE_BIT,.dstAccessMask=VK_ACCESS_SHADER_READ_BIT|VK_ACCESS_SHADER_WRITE_BIT};vkCmdPipelineBarrier(c->command,VK_PIPELINE_STAGE_HOST_BIT,VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,0,1,&before,0,NULL,0,NULL);vkCmdBindPipeline(c->command,VK_PIPELINE_BIND_POINT_COMPUTE,kernel->pipeline);vkCmdBindDescriptorSets(c->command,VK_PIPELINE_BIND_POINT_COMPUTE,kernel->layout,0,1,&set,0,NULL);if(kernel->push_bytes)vkCmdPushConstants(c->command,kernel->layout,VK_SHADER_STAGE_COMPUTE_BIT,0,kernel->push_bytes,push);bool profiled=profile_dispatch_begin(c,kernel);vkCmdDispatch(c->command,gx,gy,gz);profile_dispatch_end(c,profiled);VkMemoryBarrier after={.sType=VK_STRUCTURE_TYPE_MEMORY_BARRIER,.srcAccessMask=VK_ACCESS_SHADER_WRITE_BIT,.dstAccessMask=VK_ACCESS_HOST_READ_BIT};vkCmdPipelineBarrier(c->command,VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,VK_PIPELINE_STAGE_HOST_BIT,0,1,&after,0,NULL,0,NULL);profile_command_end(c);if((vr=vkEndCommandBuffer(c->command))!=VK_SUCCESS)return vk_error(err,"end compute command",vr);VkSubmitInfo submit={.sType=VK_STRUCTURE_TYPE_SUBMIT_INFO,.commandBufferCount=1,.pCommandBuffers=&c->command};if((vr=vkQueueSubmit(c->queue,1,&submit,c->fence))!=VK_SUCCESS)return vk_error(err,"submit compute command",vr);if((vr=vkWaitForFences(c->device,1,&c->fence,VK_TRUE,UINT64_MAX))!=VK_SUCCESS)return vk_error(err,"wait for compute command",vr);status=profile_resolve(c,err);if(!cached)vkFreeDescriptorSets(c->device,c->descriptor_pool,1,&set);return status;
}
