...
fg_status fg_vk_flush(fg_vk_context *c,fg_error *err){
    ...
    if((vr=vkQueueSubmit(c->queue,1,&submit,c->fence))!=VK_SUCCESS){fg_status status=vk_error(err,"submit flush command",vr);fg_error ignored={0};fg_vk_abort(c,&ignored);return status;}c->counters.submissions++;
    vk_fence_track(c,c->fence);
