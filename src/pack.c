...
fg_status fg_pack_run(const fg_pack_options *o,fg_error *err){
    ...
    pack_output rank[FG_RANK_COUNT]={0},ngram={0};rc=open_outputs(o,rank,&ngram,err);if(rc!=FG_OK){(void)close_outputs(rank,&ngram,err);goto done;}for(uint32_t r=0;r<FG_RANK_COUNT;r++){snprintf(m->ranks[r].endpoint,sizeof(m->ranks[r].endpoint),"192.0.2.%u:19100",42u+r);m->ranks[r].driver_reserve_bytes=512ull<<20;m->ranks[r].scratch_bytes=fg_q38_runtime_scratch_bytes(r,m->prefill_microbatch,m->prefill_window,m->max_context);}
