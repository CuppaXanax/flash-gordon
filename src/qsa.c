...
static fg_status open_decode_config(fg_qsa_session **out,fg_model *model,const char *state_path,
    ...
    *out=NULL;const fg_manifest *manifest=fg_model_manifest(model);uint32_t rank=fg_model_rank(model);
    ...
    if(rank==0u)s->locality=fg_qsa_locality_create_from_env(s->max_blocks,0u);
    for(uint32_t layer=3u;layer<FG_LAYER_COUNT;layer+=4u)
        if(rank==0u||manifest->layer_owner[layer]==rank)
            s->layers[s->layer_count++]=(uint8_t)layer;
    uint32_t expected_layers=rank==0u?FG_QSA_MAX_LAYERS:FG_QSA_OWNER_LAYERS;
    if(s->layer_count!=expected_layers){
...
fg_status fg_qsa_session_decode(fg_qsa_session *s,uint32_t layer,uint32_t token,const uint32_t position[3],const fg_vk_tensor *hidden,fg_vk_tensor **output,fg_error *err){
    ...
    uint32_t *resident_positions=fg_vk_tensor_map(s->positions);if(slot==0)memcpy(resident_positions+(uint64_t)token*3u,position,12u);else if(memcmp(resident_positions+(uint64_t)token*3u,position,12u)!=0){fg_error_set(err,FG_ERR_MISMATCH,"QSA layers received inconsistent MRoPE positions");return FG_ERR_MISMATCH;}fg_status status=fg_vk_tensor_view_rebind(s->position_view,s->positions,(uint64_t)token*12u,12u,err);fg_vk_context *vk=fg_model_vk(s->model);if(status==FG_OK&&fg_vk_profile_active(vk))status=fg_vk_profile_set_scope(vk,"qsa_projection",err);
    ...
    if(status==FG_OK&&token<30u&&!fg_vk_batch_active(vk)){const float *ap=fg_vk_tensor_map(s->attention),*op=fg_vk_tensor_map(s->output);double a2=0.0,o2=0.0;for(uint32_t i=0;i<6144u;i++)a2+=(double)ap[i]*ap[i];for(uint32_t i=0;i<2560u;i++)o2+=(double)op[i]*op[i];fprintf(stderr,"qsa[%u] t=%u selected=%u attn_rms=%.6f out_rms=%.6f\n",layer,token,token+1u,sqrt(a2/6144.0),sqrt(o2/2560.0));}
...
fg_status fg_qsa_session_prefill(fg_qsa_session *s,uint32_t layer,uint32_t first_token,const uint32_t *positions,uint32_t token_count,const fg_vk_tensor *hidden,fg_vk_tensor **output,fg_error *err){
    ...
    fg_status status=fg_vk_tensor_view_rebind(s->position_view,s->positions,(uint64_t)first_token*FG_Q38_QSA_POSITION_BYTES,(uint64_t)token_count*FG_Q38_QSA_POSITION_BYTES,err);fg_vk_context *vk=fg_model_vk(s->model);
