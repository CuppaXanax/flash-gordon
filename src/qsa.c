...
fg_status fg_qsa_session_open_decode(fg_qsa_session **out,fg_model *model,const char *state_path,uint32_t resident_tokens,fg_error *err){
    if(!out||!model||!state_path||!resident_tokens){fg_error_set(err,FG_ERR_ARGUMENT,"invalid decode QSA session arguments");return FG_ERR_ARGUMENT;}*out=NULL;const fg_manifest *manifest=fg_model_manifest(model);uint32_t rank=fg_model_rank(model);fg_qsa_session *s=calloc(1,sizeof(*s));if(!s){fg_error_set(err,FG_ERR_OOM,"allocate QSA session");return FG_ERR_OOM;}s->model=model;s->max_context=resident_tokens;s->max_blocks=(s->max_context+3u)/4u;s->max_tokens=1u;
    ...
    fg_status status=fg_qsa_state_open(&s->state,state_path,s->layers,s->layer_count,manifest->max_context,true,err);if(status==FG_OK){status=make_tensor(s,(uint64_t)resident_tokens*FG_Q38_QSA_POSITION_BYTES,&s->positions,err);}for(uint32_t i=0;status==FG_OK&&i<s->layer_count;i++){status=make_tensor(s,(uint64_t)resident_tokens*FG_Q38_QSA_INDEX_KEY_BYTES,&s->index_keys[i],err);}
    if(status==FG_OK){status=make_tensor(s,12288u*4u,&s->raw_query_gate,err);}if(status==FG_OK){status=make_tensor(s,512u*4u,&s->raw_key,err);}if(status==FG_OK){status=make_tensor(s,512u*4u,&s->raw_value,err);}if(status==FG_OK){status=make_tensor(s,6144u*4u,&s->query,err);}if(status==FG_OK){status=make_tensor(s,6144u*4u,&s->gate,err);}if(status==FG_OK){status=make_tensor(s,512u*4u,&s->key,err);}if(status==FG_OK){status=make_tensor(s,512u*4u,&s->raw_index_query,err);}if(status==FG_OK){status=make_tensor(s,128u*4u,&s->raw_index_key,err);}if(status==FG_OK){status=make_tensor(s,512u*4u,&s->index_query,err);}if(status==FG_OK){status=make_tensor(s,FG_Q38_QSA_KEY_BYTES,&s->key_q8,err);}if(status==FG_OK){status=make_tensor(s,FG_Q38_QSA_VALUE_BYTES,&s->value_q4,err);}if(status==FG_OK){status=make_tensor(s,FG_Q38_QSA_INDEX_KEY_BYTES,&s->index_key_q8,err);}
    for(uint32_t i=0;status==FG_OK&&i<2u;i++){status=make_tensor(s,(uint64_t)s->max_blocks*4u,&s->scores[i],err);if(status==FG_OK)status=make_tensor(s,(uint64_t)s->max_blocks*4u,&s->ids[i],err);}
    if(status==FG_OK){status=make_tensor(s,(uint64_t)FG_QSA_SELECTED_TOKENS*FG_Q38_QSA_TOKEN_RECORD_BYTES,&s->selected_records,err);}if(status==FG_OK){status=make_tensor(s,6144u*4u,&s->attention,err);}if(status==FG_OK){status=make_tensor(s,2560u*4u,&s->output,err);}
    ...
    fprintf(stderr,"[rank %u] QSA decode session: %u tokens, %u layers, %.1f MiB GPU\n",rank,resident_tokens,s->layer_count,(double)((uint64_t)resident_tokens*(FG_Q38_QSA_POSITION_BYTES+s->layer_count*FG_Q38_QSA_INDEX_KEY_BYTES))/(1024.0*1024.0));
    if(status!=FG_OK){fg_qsa_session_close(s);return status;}*out=s;return FG_OK;
}
...
void fg_qsa_session_set_tokens(fg_qsa_session *s,uint32_t tokens){if(!s)return;for(uint32_t i=0;i<s->layer_count;i++)fg_qsa_state_set_layer_tokens(s->state,i,tokens);}
...
static fg_status commit_and_attend(fg_qsa_session *s,uint32_t slot,uint32_t token,const uint32_t position[3],const uint8_t *key,const uint8_t *value,const uint8_t *index_key,const fg_vk_tensor *index_query,const fg_vk_tensor *query,const fg_vk_tensor *gate,fg_vk_tensor *attention,fg_error *err){
    ...
    fg_status status=fg_qsa_state_write_block(s->state,slot,token/FG_Q38_QSA_COMPRESS_RATIO,s->partial[slot],inside+1u,err);if(status!=FG_OK)return status;memcpy((uint8_t *)fg_vk_tensor_map(s->index_keys[slot])+(uint64_t)token*FG_Q38_QSA_INDEX_KEY_BYTES,index_key,FG_Q38_QSA_INDEX_KEY_BYTES);
