...
struct fg_fabric{uint32_t rank;bool direct_io;fg_uring *ring;fabric_channel peer[FG_RANK_COUNT][2];};
...
fg_status fg_fabric_open(fg_fabric **out,const fg_manifest *m,uint32_t rank,fg_error *err){
    if(!out||!m||rank>=FG_RANK_COUNT){fg_error_set(err,FG_ERR_ARGUMENT,"invalid fabric arguments");return FG_ERR_ARGUMENT;}*out=NULL;fg_fabric *f=calloc(1,sizeof(*f));if(!f){fg_error_set(err,FG_ERR_OOM,"allocate fabric");return FG_ERR_OOM;}const char *io=getenv("FG_FABRIC_IO");if(!io||!*io||strcmp(io,"uring")==0)f->direct_io=false;else if(strcmp(io,"direct")==0)f->direct_io=true;else{fg_error_set(err,FG_ERR_ARGUMENT,"unknown FG_FABRIC_IO %.32s",io);free(f);return FG_ERR_ARGUMENT;}f->rank=rank;for(uint32_t p=0;p<FG_RANK_COUNT;p++)for(uint32_t c=0;c<2;c++)f->peer[p][c].fd=-1;struct in_addr local;uint16_t port;if(!parse_endpoint(m->ranks[rank].endpoint,&local,&port)){fg_error_set(err,FG_ERR_FORMAT,"invalid local rank endpoint");free(f);return FG_ERR_FORMAT;}int listeners[2]={make_listener(local,port),make_listener(local,(uint16_t)(port+1))};if(listeners[0]<0||listeners[1]<0){fg_error_set(err,FG_ERR_IO,"listen on rank %u endpoint: %s",rank,strerror(errno));close_fd(&listeners[0]);close_fd(&listeners[1]);free(f);return FG_ERR_IO;}fg_status rc=FG_OK;
...
bool fg_fabric_uses_direct_io(const fg_fabric *f){return f&&f->direct_io;}
static fg_status direct_socket_all(fg_fabric *f,uint32_t peer,fg_fabric_class cls,void *buffer,uint32_t bytes,bool send_mode,fg_error *err){if(sync_all(f->peer[peer][cls].fd,buffer,bytes,send_mode))return FG_OK;fg_error_set(err,FG_ERR_IO,"%s rank %u channel %u: %s",send_mode?"send":"receive",peer,cls,errno?strerror(errno):"peer closed socket");return FG_ERR_IO;}
fg_status fg_fabric_send(fg_fabric *f,uint32_t peer,fg_fabric_class cls,fg_message_type type,uint64_t req,uint32_t seq,uint32_t flags,const void *payload,uint32_t bytes,fg_error *err){
    if(!f||peer>=FG_RANK_COUNT||peer==f->rank||cls>FG_FABRIC_BULK){fg_error_set(err,FG_ERR_ARGUMENT,"invalid fabric destination");return FG_ERR_ARGUMENT;}fg_frame_header h;fg_status rc=fg_frame_encode(&h,type,req,seq,flags,payload,bytes,err);if(rc!=FG_OK)return rc;
    if(bytes&&bytes<=65536u-sizeof(h)){uint8_t combined[65536u];memcpy(combined,&h,sizeof(h));memcpy(combined+sizeof(h),payload,bytes);return f->direct_io?direct_socket_all(f,peer,cls,combined,(uint32_t)sizeof(h)+bytes,true,err):fg_uring_send_all(f->ring,f->peer[peer][cls].fixed_slot,combined,(uint32_t)sizeof(h)+bytes,err);}
    rc=f->direct_io?direct_socket_all(f,peer,cls,&h,sizeof(h),true,err):fg_uring_send_all(f->ring,f->peer[peer][cls].fixed_slot,&h,sizeof(h),err);if(rc==FG_OK&&bytes)rc=f->direct_io?direct_socket_all(f,peer,cls,(void *)payload,bytes,true,err):fg_uring_send_all(f->ring,f->peer[peer][cls].fixed_slot,payload,bytes,err);return rc;
}
fg_status fg_fabric_recv(fg_fabric *f,uint32_t peer,fg_fabric_class cls,fg_frame_header *h,void *payload,uint32_t cap,uint32_t *bytes,fg_error *err){
    if(!f||peer>=FG_RANK_COUNT||peer==f->rank||cls>FG_FABRIC_BULK||!h){fg_error_set(err,FG_ERR_ARGUMENT,"invalid fabric receive");return FG_ERR_ARGUMENT;}fg_status rc=f->direct_io?direct_socket_all(f,peer,cls,h,sizeof(*h),false,err):fg_uring_recv_all(f->ring,f->peer[peer][cls].fixed_slot,h,sizeof(*h),err);uint32_t n=rc==FG_OK?ntohl(h->bytes_be):0;if(rc==FG_OK&&(n>cap||(n&&!payload))){fg_error_set(err,FG_ERR_LIMIT,"fabric payload %u exceeds receive buffer %u",n,cap);return FG_ERR_LIMIT;}if(rc==FG_OK&&n)rc=f->direct_io?direct_socket_all(f,peer,cls,payload,n,false,err):fg_uring_recv_all(f->ring,f->peer[peer][cls].fixed_slot,payload,n,err);if(rc==FG_OK)rc=fg_frame_validate(h,payload,bytes,err);return rc;
}
