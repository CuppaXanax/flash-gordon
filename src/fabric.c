...
fg_status fg_fabric_recv(fg_fabric *f,uint32_t peer,fg_fabric_class cls,fg_frame_header *h,void *payload,uint32_t cap,uint32_t *bytes,fg_error *err){if(!f||peer>=FG_RANK_COUNT||peer==f->rank||cls>FG_FABRIC_BULK||!h){fg_error_set(err,FG_ERR_ARGUMENT,"invalid fabric receive");return FG_ERR_ARGUMENT;}fg_status rc=fg_uring_recv_all(f->ring,f->peer[peer][cls].fixed_slot,h,sizeof(*h),err);uint32_t n=rc==FG_OK?ntohl(h->bytes_be):0;if(rc==FG_OK&&(n>cap||(n&&!payload))){fg_error_set(err,FG_ERR_LIMIT,"fabric payload %u exceeds receive buffer %u",n,cap);return FG_ERR_LIMIT;}if(rc==FG_OK&&n)rc=fg_uring_recv_all(f->ring,f->peer[peer][cls].fixed_slot,payload,n,err);if(rc==FG_OK)rc=fg_frame_validate(h,payload,bytes,err);return rc;}
...
fg_status fg_fabric_prep_frame_send(fg_fabric *f,uint32_t peer,fg_fabric_class cls,const void *frame,uint32_t bytes,uint64_t tag,fg_error *err){
    if(!f||peer>=FG_RANK_COUNT||peer==f->rank||cls>FG_FABRIC_BULK||!frame||!bytes){fg_error_set(err,FG_ERR_ARGUMENT,"invalid async frame send");return FG_ERR_ARGUMENT;}
    return fg_uring_prep_send(f->ring,f->peer[peer][cls].fixed_slot,frame,bytes,tag,err);
}
fg_status fg_fabric_prep_frame_recv(fg_fabric *f,uint32_t peer,fg_fabric_class cls,void *frame,uint32_t bytes,uint64_t tag,fg_error *err){
    if(!f||peer>=FG_RANK_COUNT||peer==f->rank||cls>FG_FABRIC_BULK||!frame||!bytes){fg_error_set(err,FG_ERR_ARGUMENT,"invalid async frame recv");return FG_ERR_ARGUMENT;}
    return fg_uring_prep_recv(f->ring,f->peer[peer][cls].fixed_slot,frame,bytes,tag,err);
}
