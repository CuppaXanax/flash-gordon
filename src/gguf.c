...
fg_tensor_kind fg_gguf_tensor_kind(const char *name){if(strstr(name,"per_layer_token_embd"))return FG_TENSOR_NGRAM;if(strncmp(name,"v.",2u)==0||strncmp(name,"mm.",3u)==0||strstr(name,"vision")||strstr(name,"visual"))return FG_TENSOR_VISION;if(strncmp(name,"blk.48.",7u)==0||strstr(name,"nextn")||strstr(name,"mtp")||strstr(name,"draft"))return FG_TENSOR_MTP;if(strstr(name,"_exps")||strstr(name,"experts"))return FG_TENSOR_ROUTED_EXPERT;return FG_TENSOR_COMMON;}
