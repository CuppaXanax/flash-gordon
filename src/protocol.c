...
static fg_status validate_ngram_route(uint8_t source,uint8_t destination,uint8_t head_begin,uint8_t head_count,fg_error *err){if(source>=FG_RANK_COUNT||destination>=FG_RANK_COUNT||source==destination||!head_count||head_count>FG_NGRAM_SHARD_MAX_HEADS||head_begin>=FG_NGRAM_HEAD_COUNT||head_count>FG_NGRAM_HEAD_COUNT-head_begin){fg_error_set(err,FG_ERR_FORMAT,"invalid resident n-gram route");return FG_ERR_FORMAT;}return FG_OK;}

fg_status fg_ngram_work_encode(uint8_t *output,uint32_t capacity,uint32_t *bytes,const fg_ngram_work *work,fg_error *err){if(!output||!bytes||!work){fg_error_set(err,FG_ERR_ARGUMENT,"invalid n-gram work output");return FG_ERR_ARGUMENT;}fg_status status=validate_ngram_route(work->source_rank,work->destination_rank,work->head_begin,work->head_count,err);uint32_t required=8u+(uint32_t)work->head_count*8u;if(status!=FG_OK)return status;if(capacity<required){fg_error_set(err,FG_ERR_LIMIT,"n-gram work buffer is too small");return FG_ERR_LIMIT;}output[0]=work->source_rank;output[1]=work->destination_rank;output[2]=work->head_begin;output[3]=work->head_count;put_u32_be(output+4u,work->token_index);for(uint32_t i=0;i<work->head_count;i++)put_u64_be(output+8u+i*8u,work->rows[i]);*bytes=required;return FG_OK;}

fg_status fg_ngram_work_decode(fg_ngram_work *work,const uint8_t *payload,uint32_t bytes,fg_error *err){if(!work||!payload||bytes<8u){fg_error_set(err,FG_ERR_ARGUMENT,"invalid n-gram work input");return FG_ERR_ARGUMENT;}uint8_t head_count=payload[3];uint32_t required=8u+(uint32_t)head_count*8u;if(bytes!=required){fg_error_set(err,FG_ERR_FORMAT,"invalid n-gram work size");return FG_ERR_FORMAT;}memset(work,0,sizeof(*work));work->source_rank=payload[0];work->destination_rank=payload[1];work->head_begin=payload[2];work->head_count=head_count;work->token_index=get_u32_be(payload+4u);for(uint32_t i=0;i<head_count;i++)work->rows[i]=get_u64_be(payload+8u+i*8u);return validate_ngram_route(work->source_rank,work->destination_rank,work->head_begin,work->head_count,err);}

fg_status fg_ngram_result_encode(uint8_t *output,uint32_t capacity,uint32_t *bytes,const fg_ngram_result *result,fg_error *err){if(!output||!bytes||!result){fg_error_set(err,FG_ERR_ARGUMENT,"invalid n-gram result output");return FG_ERR_ARGUMENT;}fg_status status=validate_ngram_route(result->source_rank,result->destination_rank,result->head_begin,result->head_count,err);uint32_t packed_bytes=(uint32_t)result->head_count*FG_NGRAM_WIRE_ROW_BYTES,required=8u+packed_bytes;if(status!=FG_OK)return status;if(capacity<required){fg_error_set(err,FG_ERR_LIMIT,"n-gram result buffer is too small");return FG_ERR_LIMIT;}output[0]=result->source_rank;output[1]=result->destination_rank;output[2]=result->head_begin;output[3]=result->head_count;put_u32_be(output+4u,result->token_index);memcpy(output+8u,result->packed,packed_bytes);*bytes=required;return FG_OK;}

fg_status fg_ngram_result_decode(fg_ngram_result *result,const uint8_t *payload,uint32_t bytes,fg_error *err){if(!result||!payload||bytes<8u){fg_error_set(err,FG_ERR_ARGUMENT,"invalid n-gram result input");return FG_ERR_ARGUMENT;}uint8_t head_count=payload[3];uint32_t packed_bytes=(uint32_t)head_count*FG_NGRAM_WIRE_ROW_BYTES,required=8u+packed_bytes;if(bytes!=required){fg_error_set(err,FG_ERR_FORMAT,"invalid n-gram result size");return FG_ERR_FORMAT;}memset(result,0,sizeof(*result));result->source_rank=payload[0];result->destination_rank=payload[1];result->head_begin=payload[2];result->head_count=head_count;result->token_index=get_u32_be(payload+4u);memcpy(result->packed,payload+8u,packed_bytes);return validate_ngram_route(result->source_rank,result->destination_rank,result->head_begin,result->head_count,err);}
