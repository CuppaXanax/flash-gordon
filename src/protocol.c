...
fg_status fg_layer_result_encode(uint8_t output[FG_LAYER_RESULT_BYTES],const fg_layer_result *result,uint32_t *bytes,fg_error *err){if(!output||!bytes){fg_error_set(err,FG_ERR_ARGUMENT,"layer result output is null");return FG_ERR_ARGUMENT;}fg_status status=validate_layer_result(result,err);if(status!=FG_OK)return status;output[0]=result->layer;output[1]=result->source_rank;output[2]=result->destination_rank;output[3]=0u;put_u32_be(output+4u,result->token_index);uint32_t offset=8u;for(uint32_t i=0;i<FG_HYPER_WIDTH;i++,offset+=4u)put_f32_be(output+offset,result->hyper[i]);*bytes=FG_LAYER_RESULT_BYTES;return FG_OK;}

fg_status fg_layer_result_decode(fg_layer_result *result,const uint8_t *payload,uint32_t bytes,fg_error *err){if(!result||!payload){fg_error_set(err,FG_ERR_ARGUMENT,"invalid layer result input");return FG_ERR_ARGUMENT;}if(payload[3]||bytes!=FG_LAYER_RESULT_BYTES){fg_error_set(err,FG_ERR_FORMAT,"invalid layer result payload size or reserved byte");return FG_ERR_FORMAT;}memset(result,0,sizeof(*result));result->layer=payload[0];result->source_rank=payload[1];result->destination_rank=payload[2];result->token_index=get_u32_be(payload+4u);uint32_t offset=8u;for(uint32_t i=0;i<FG_HYPER_WIDTH;i++,offset+=4u)result->hyper[i]=get_f32_be(payload+offset);return validate_layer_result(result,err);}
...
static fg_status validate_decode_batch_work(const fg_decode_batch_work *work,fg_error *err){
    if(!work||work->layer>=FG_LAYER_COUNT||work->source_rank>=FG_RANK_COUNT||
       ...
       (work->flags&~(FG_LAYER_WORK_HAS_NGRAM|FG_LAYER_WORK_FLAG_OUTPUT_4WAY_GREEDY|FG_LAYER_WORK_FLAG_OUTPUT_RELAY))||
...
fg_status fg_output_slice_hidden_encode(uint8_t output[FG_OUTPUT_SLICE_HIDDEN_BYTES],
    ...
    output[2]=0u;output[3]=0u;put_u32_be(output+4u,slice->token_index);
    uint32_t offset=8u;
    for(uint32_t i=0;i<FG_HIDDEN_SIZE;i++,offset+=4u)put_f32_be(output+offset,slice->hidden[i]);
    *bytes=FG_OUTPUT_SLICE_HIDDEN_BYTES;
    return FG_OK;
}
...
fg_status fg_output_slice_hidden_decode(fg_output_slice_hidden *slice,
    ...
    if(bytes!=FG_OUTPUT_SLICE_HIDDEN_BYTES||payload[2]||payload[3]){
    ...
    for(uint32_t i=0,offset=8u;i<FG_HIDDEN_SIZE;i++,offset+=4u)slice->hidden[i]=get_f32_be(payload+offset);
...
fg_status fg_output_history_encode(uint8_t *output,uint32_t capacity,uint32_t *bytes,
    ...
    if(!output||!bytes||!history||history->count>FG_NATIVE_CONTEXT||
       ...
       history->session_slot>=FG_DECODE_BATCH_MAX_SLOTS){
    ...
    put_u32_be(output,history->count);output[4]=history->session_slot;
    output[5]=0u;output[6]=0u;output[7]=0u;
...
fg_status fg_output_history_decode(fg_output_history *history,uint32_t *storage,
    ...
    uint32_t count=get_u32_be(payload);
    uint8_t session_slot=payload[4];
    uint32_t reserved=((uint32_t)payload[5]<<16u)|((uint32_t)payload[6]<<8u)|payload[7];
    uint64_t required=FG_OUTPUT_HISTORY_HEADER_BYTES+(uint64_t)count*4u;
    if(reserved||session_slot>=FG_DECODE_BATCH_MAX_SLOTS||count>FG_NATIVE_CONTEXT||
       count>capacity||required!=bytes||
       (count&&!storage)){
    ...
    history->tokens=storage;history->count=count;history->session_slot=session_slot;return FG_OK;
}
...
fg_status fg_owner_session_control_decode(fg_owner_session_control *control,
    ...
    if(bytes!=FG_OWNER_SESSION_CONTROL_BYTES||payload[7]||
