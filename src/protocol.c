...
fg_status fg_frame_validate(const fg_frame_header *h,const void *payload,uint32_t *payload_bytes,fg_error *err){
    ...
    uint16_t type=bswap16(h->type_be);if(type<FG_MSG_HELLO||type>FG_MSG_PREFILL_LAYER_RESULT||bytes>FG_MAX_FRAME_BYTES||(bytes&&!payload)){fg_error_set(err,FG_ERR_FORMAT,"invalid frame type or length");return FG_ERR_FORMAT;}
...
fg_status fg_prefill_results_validate_route(const fg_manifest *manifest,uint32_t layer,uint32_t first_position,uint32_t owner_rank,uint16_t token_count,const uint16_t *expert_ids,const fg_prefill_result *results,uint32_t result_count,fg_error *err){
    if(!manifest||layer>=FG_LAYER_COUNT||owner_rank>=FG_RANK_COUNT||manifest->layer_owner[layer]!=owner_rank||!token_count||token_count>FG_PREFILL_MAX_TOKENS||!expert_ids||!results||!result_count||result_count>FG_GROUP_SIZE){fg_error_set(err,FG_ERR_ARGUMENT,"invalid prefill result route validation arguments");return FG_ERR_ARGUMENT;}
