...
`X-Flash-Gordon-Prefix-Cache: hit`, `X-Flash-Gordon-Reset-Reason: none`, reused
tokens close to the turn-1 prompt size, and only the suffix prefill. On the
baseline binary the same script reports `miss` + `cold-start` with the full
prompt re-prefilled.
