...
static void test_expert_map_single(void){fg_manifest *m=malloc(sizeof(*m));CHECK(m!=NULL);if(!m)return;fg_manifest_init(m);char path[128];snprintf(path,sizeof(path),"/tmp/fg-single-map-%ld.txt",(long)getpid());FILE *file=fopen(path,"w");CHECK(file!=NULL);if(file){fputs("# Flash Gordon route placement v1\n",file);for(uint32_t l=0;l<FG_LAYER_COUNT;l++){fprintf(file,"layer=%u ranks=",l);for(uint32_t e=0;e<FG_EXPERT_COUNT;e++)fprintf(file,"%s%u",e?",":"",l%FG_RANK_COUNT);fputc('\n',file);}fclose(file);fg_error err={0};CHECK(fg_topology_assign_map_file(m,path,&err)==FG_OK);CHECK(m->expert_rank[3][511]==3u);CHECK(m->layer_groups[3][0]==3u&&m->layer_groups[3][3]==3u);CHECK(fg_topology_layer_rank_experts(m,3,3)==FG_EXPERT_COUNT);CHECK(fg_topology_layer_rank_experts(m,3,4)==0u);CHECK(fg_topology_validate(m,&err)==FG_OK);}unlink(path);free(m);}

static void test_expert_map_owners(void){fg_manifest *m=malloc(sizeof(*m));CHECK(m!=NULL);if(!m)return;fg_manifest_init(m);char path[128];snprintf(path,sizeof(path),"/tmp/fg-owner-map-%ld.txt",(long)getpid());FILE *file=fopen(path,"w");CHECK(file!=NULL);if(file){fputs("# Flash Gordon route placement v1\n",file);fputs("owners=",file);for(uint32_t l=0;l<FG_LAYER_COUNT;l++)fprintf(file,"%s%u",l?",":"",l/6u);fputc('\n',file);for(uint32_t l=0;l<FG_LAYER_COUNT;l++){fprintf(file,"layer=%u ranks=",l);for(uint32_t e=0;e<FG_EXPERT_COUNT;e++)fprintf(file,"%s%u",e?",":"",l/6u);fputc('\n',file);}fclose(file);fg_error err={0};CHECK(fg_topology_assign_map_file(m,path,&err)==FG_OK);CHECK(m->layer_owner[0]==0u&&m->layer_owner[5]==0u&&m->layer_owner[6]==1u&&m->layer_owner[47]==7u);CHECK(fg_topology_validate(m,&err)==FG_OK);}unlink(path);free(m);}
...
static void test_decode_layer_protocol(void){
    ...
    if(result&&result_decoded&&result_wire){
        ...
        CHECK(fg_decode_layer_result_encode(result_wire,result,&bytes,&err)==FG_OK);
...
static void test_ngram_suffix(void){
    enum{HISTORY=131200,CHUNK=128};
    int32_t *history=malloc(HISTORY*sizeof(*history));CHECK(history!=NULL);if(!history)return;
    for(uint32_t i=0;i<HISTORY;i++)history[i]=(int32_t)((i*37u+19u)%FG_Q38_VOCAB_SIZE);
    const uint32_t boundaries[]={0u,1u,2u,126u,127u,128u,131070u,131071u};
    for(uint32_t i=0;i<sizeof(boundaries)/sizeof(boundaries[0]);i++)
        history[boundaries[i]]=FG_Q38_EOS_TOKEN;
    uint64_t rows[CHUNK*FG_NGRAM_HEAD_COUNT],addresses[CHUNK*FG_NGRAM_HEAD_COUNT];
    uint64_t expected[FG_NGRAM_HEAD_COUNT],expected_addresses[FG_NGRAM_HEAD_COUNT];
    const uint32_t starts[]={0u,1u,2u,127u,128u,131068u};fg_error err={0};
    for(uint32_t s=0;s<sizeof(starts)/sizeof(starts[0]);s++){
        uint32_t first=starts[s];
        CHECK(fg_q38_ngram_lookup_range(history,HISTORY,first,CHUNK,rows,addresses,&err)==FG_OK);
        for(uint32_t i=0;i<CHUNK;i++){
            CHECK(fg_q38_ngram_lookup(history,first+i+1u,expected,expected_addresses,&err)==FG_OK);
            CHECK(!memcmp(rows+i*FG_NGRAM_HEAD_COUNT,expected,sizeof(expected)));
            CHECK(!memcmp(addresses+i*FG_NGRAM_HEAD_COUNT,expected_addresses,sizeof(expected_addresses)));
        }
    }
    /* Divergence and rollback require no cached boundary to invalidate. */
    history[126u]=17;history[127u]=18;history[128u]=19;
    CHECK(fg_q38_ngram_lookup_range(history,129u,128u,1u,rows,addresses,&err)==FG_OK);
    CHECK(fg_q38_ngram_lookup(history,129u,expected,expected_addresses,&err)==FG_OK);
    CHECK(!memcmp(rows,expected,sizeof(expected)));
    history[0]=-1; /* Outside the dependency window of the validated prefix. */
    CHECK(fg_q38_ngram_lookup_range(history,HISTORY,131068u,CHUNK,rows,addresses,&err)==FG_OK);
    history[131066u]=-1;
    CHECK(fg_q38_ngram_lookup_range(history,HISTORY,131068u,CHUNK,rows,addresses,&err)==FG_ERR_FORMAT);
    CHECK(fg_q38_ngram_lookup_range(history,HISTORY,HISTORY,1u,rows,addresses,&err)==FG_ERR_ARGUMENT);
    CHECK(fg_q38_ngram_lookup_range(history,HISTORY,SIZE_MAX,1u,rows,addresses,&err)==FG_ERR_ARGUMENT);
    free(history);
}
...
static void test_qsa_scratch_geometry(void){
    ...
    const uint32_t batches[]={32u,64u,128u,256u};
    for(uint32_t i=0;i<4u;i++){
        uint32_t batch=batches[i];
        uint64_t end=fg_align_up_u64(fg_qsa_attention_scratch_bytes(batch),FG_ALIGNMENT)+
            fg_qsa_selection_scratch_bytes(262144u,batch);
        const uint64_t live[]={batch*2560u*4u,batch*FG_GROUP_SIZE*4u,
                               batch*FG_HIDDEN_SIZE*4u,batch*4u};
        for(uint32_t j=0;j<4u;j++)end=fg_align_up_u64(end+live[j],FG_ALIGNMENT);
        CHECK(end<=fg_qsa_attention_family_scratch_bytes(batch));
    }
}
...
static void test_pack_cooked_experts(void){
    enum{GATE_INPUT=256,GATE_OUTPUT=8,GATE_BLOCK=144,DOWN_INPUT=32,DOWN_OUTPUT=16,DOWN_BLOCK=24,Q8_INPUT=64,Q8_OUTPUT=32};
    const uint64_t gate_expert=(uint64_t)GATE_OUTPUT*GATE_BLOCK,gate_bytes=gate_expert*FG_EXPERT_COUNT,down_expert=(uint64_t)DOWN_OUTPUT*DOWN_BLOCK,down_bytes=down_expert*FG_EXPERT_COUNT,q8_raw=(uint64_t)(Q8_INPUT/FG_QK8_0)*FG_Q8_0_BLOCK_BYTES*Q8_OUTPUT,q8_expert=fg_q8_0_cooked_matrix_bytes(Q8_INPUT,Q8_OUTPUT),q8_bytes=q8_expert*FG_EXPERT_COUNT;
    CHECK(q8_expert==q8_raw);
    ...
    put_u32(file,0x46554747u);put_u32(file,3u);put_u64(file,3u);put_u64(file,0u);put_string(file,"blk.0.ffn_gate_exps.weight");put_u32(file,3u);put_u64(file,GATE_INPUT);put_u64(file,GATE_OUTPUT);put_u64(file,FG_EXPERT_COUNT);put_u32(file,12u);put_u64(file,0u);put_string(file,"blk.0.ffn_down_exps.weight");put_u32(file,3u);put_u64(file,DOWN_INPUT);put_u64(file,DOWN_OUTPUT);put_u64(file,FG_EXPERT_COUNT);put_u32(file,7u);put_u64(file,gate_bytes);put_string(file,"blk.1.ffn_down_exps.weight");put_u32(file,3u);put_u64(file,Q8_INPUT);put_u64(file,Q8_OUTPUT);put_u64(file,FG_EXPERT_COUNT);put_u32(file,8u);put_u64(file,gate_bytes+down_bytes);while((ftell(file)&31)!=0)fputc(0,file);for(uint64_t i=0;i<gate_bytes;i++)fputc((int)(i*29u+7u),file);for(uint64_t i=0;i<down_bytes;i++)fputc((int)(i*31u+11u),file);for(uint64_t i=0;i<q8_bytes;i++)fputc((int)(i*37u+13u),file);fclose(file);
    const char *sources[]={source};fg_pack_options options={.output_dir=directory,.source_paths=sources,.source_count=1u,.skip_model_validation=true};fg_error error={0};CHECK(fg_pack_run(&options,&error)==FG_OK);fg_manifest *manifest=malloc(sizeof(*manifest));CHECK(manifest!=NULL);if(manifest){CHECK(fg_manifest_read(manifest_path,manifest,&error)==FG_OK);uint32_t gate_count=0,down_count=0,q8_count=0;for(uint32_t i=0;i<manifest->tensor_count;i++){const fg_tensor_record *record=&manifest->tensors[i];if(strstr(record->name,"blk.0.ffn_gate_exps")){gate_count++;CHECK(record->layout==FG_TENSOR_LAYOUT_K_QUANT_EXPERT_COOKED&&record->bytes==gate_expert*FG_EXPERTS_PER_RANK);}if(strstr(record->name,"blk.0.ffn_down_exps")){down_count++;CHECK(record->layout==FG_TENSOR_LAYOUT_Q5_1_EXPERT_COOKED&&record->bytes==down_expert*FG_EXPERTS_PER_RANK);}if(strstr(record->name,"blk.1.ffn_down_exps")){q8_count++;CHECK(record->layout==FG_TENSOR_LAYOUT_Q8_0_EXPERT_COOKED&&record->ggml_type==8u&&record->bytes==q8_expert*FG_EXPERTS_PER_RANK);}}CHECK(gate_count==FG_GROUP_SIZE&&down_count==FG_GROUP_SIZE&&q8_count==FG_GROUP_SIZE);fg_verify_options verify={.manifest_path=manifest_path,.pack_dir=directory,.source_paths=sources,.source_count=1u};CHECK(fg_pack_verify(&verify,&error)==FG_OK);free(manifest);}
    unlink(manifest_path);for(uint32_t rank=0;rank<FG_RANK_COUNT;rank++){snprintf(path,sizeof(path),"%s/rank-%02u.fgw",directory,rank);unlink(path);}snprintf(path,sizeof(path),"%s/ngram.iq4nl",directory);unlink(path);rmdir(directory);unlink(source);
}
...
static void test_prefill_protocol(void){
    ...
    if(outputs&&decoded_outputs&&result_wire){for(uint32_t i=0;i<PAIRS*FG_HIDDEN_SIZE;i++)outputs[i]=sinf((float)i*0.0007f);fg_prefill_result result={.layer=9,.source_rank=3,.destination_rank=1,.first_position=4096,.token_count=TOKENS,.pair_count=PAIRS,.pairs=result_pairs,.outputs=outputs,.contributor_mask=1u<<3u},decoded_result={0};uint32_t result_bytes=0;CHECK(fg_prefill_result_encode(result_wire,FG_PREFILL_RESULT_MAX_BYTES,&result_bytes,&result,&err)==FG_OK);CHECK(result_bytes==FG_PREFILL_RESULT_HEADER_BYTES+PAIRS*FG_PREFILL_RESULT_PAIR_BYTES+TOKENS*FG_HIDDEN_SIZE*4u);CHECK(fg_prefill_result_decode(&decoded_result,decoded_result_pairs,PAIRS,decoded_outputs,(uint64_t)PAIRS*FG_HIDDEN_SIZE,result_wire,result_bytes,&err)==FG_OK);CHECK(decoded_result.layer==9&&decoded_result.first_position==4096&&decoded_result_pairs[4].token_slot==2&&decoded_result_pairs[4].routing_slot==7&&decoded_outputs[TOKENS*FG_HIDDEN_SIZE-1u]==outputs[TOKENS*FG_HIDDEN_SIZE-1u]);result_wire[3]=0u;CHECK(fg_prefill_result_decode(&decoded_result,decoded_result_pairs,PAIRS,decoded_outputs,(uint64_t)PAIRS*FG_HIDDEN_SIZE,result_wire,result_bytes,&err)==FG_ERR_FORMAT);result_wire[3]=FG_PREFILL_RESULT_VERSION;result_wire[15]=1u;CHECK(fg_prefill_result_decode(&decoded_result,decoded_result_pairs,PAIRS,decoded_outputs,(uint64_t)PAIRS*FG_HIDDEN_SIZE,result_wire,result_bytes,&err)==FG_ERR_FORMAT);}
...
static void test_output_history_protocol(void){
    ...
    CHECK(fg_frame_validate_version(&frame,FG_PROTOCOL_VERSION,
