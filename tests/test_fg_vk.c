...
static int run_topk_path(uint32_t count,const float *scores,const uint32_t *ids,
                         float *out_scores,uint32_t *out_ids,uint32_t *produced){
...
static bool topk_before(float a,uint32_t ai,float b,uint32_t bi){
    bool ap=ai==0xffffffffu,bp=bi==0xffffffffu;
    if(ap||bp)return !ap&&bp;
    bool an=isnan(a)||isinf(a),bn=isnan(b)||isinf(b);
    if(an||bn)return an!=bn?an:ai<bi;
    return a>b||(a==b&&ai<bi);
}

static int test_topk_reduce_chunked(void){
    ...
    float *scores=malloc((size_t)max*4u),*got_scores=malloc((size_t)max*4u);
    uint32_t *ids=malloc((size_t)max*4u),*got_ids=malloc((size_t)max*4u);
    int ok=scores&&got_scores&&ids&&got_ids;
    ...
    for(uint32_t c=0;ok&&c<sizeof(counts)/sizeof(counts[0]);c++){
        uint32_t count=counts[c],produced=0,expected=((count+4095u)/4096u)*512u;
        if(((count+4095u)/4096u)==1u&&count<512u)expected=count;
        ok=run_topk_path(count,scores,ids,got_scores,got_ids,&produced)&&produced==expected;
        for(uint32_t i=0;ok&&i<produced;i++){
            uint32_t group=i/512u,within=i%512u;
            uint32_t remaining=count>group*4096u?count-group*4096u:0u;
            uint32_t valid=remaining>512u?512u:remaining;
            if(within>=valid){
                if(got_scores[i]!=-INFINITY||got_ids[i]!=0xffffffffu)ok=0;
            }else{
                uint32_t id=got_ids[i];
                if(id>=count||memcmp(&got_scores[i],&scores[id],sizeof(float))!=0)ok=0;
                else if(within+1u<valid&&topk_before(got_scores[i+1u],got_ids[i+1u],
                                                     got_scores[i],got_ids[i]))ok=0;
            }
        }
        if(!ok)fprintf(stderr,"topk chunked count=%u produced=%u expected=%u\n",
                       count,produced,expected);
    }
    free(got_ids);free(ids);free(got_scores);free(scores);
    return ok;
}
...
static int run_expert_decode_fused(uint32_t gate_type,uint32_t down_type,bool down_cooked){
    ...
    for(uint32_t slot=0;slot<SLOTS;slot++){
        schedule[slot*9u]=(slot+1u)%EXPERTS;      /* local expert, not identity */
        schedule[slot*9u+1u]=(slot*3u+1u)%SLOTS;  /* routing slot, permuted */
        gates[slot]=0.6f-(float)slot*0.25f;
    }
    fg_vk_tensor *gw=NULL,*uw=NULL,*dw=NULL;
    fg_vk_tensor *activation=tensor(q8,sizeof(q8)),*tiles=tensor(schedule,sizeof(schedule)),*gate_values=tensor(gates,sizeof(gates));
    fg_vk_tensor *gate=tensor(NULL,(uint64_t)SLOTS*MID*4u),*up=tensor(NULL,(uint64_t)SLOTS*MID*4u),*mid=tensor(NULL,(uint64_t)SLOTS*MID*4u);
    fg_vk_tensor *down=tensor(NULL,(uint64_t)SLOTS*HIDDEN*4u),*reference=tensor(NULL,(uint64_t)HIDDEN*4u),*fused=tensor(NULL,(uint64_t)HIDDEN*4u);
    int ok=activation&&tiles&&gate_values&&gate&&up&&mid&&down&&reference&&fused;
    ...
    if(ok)ok=fg_vk_begin(context,&error)==FG_OK&&
        fg_vk_moe_decode_gate_up(context,mid,gw,uw,activation,tiles,MID,HIDDEN,(uint32_t)gate_stride,(uint32_t)up_stride,gate_type,up_type,SLOTS,&error)==FG_OK&&
        fg_vk_moe_decode_down_reduce(context,fused,dw,tiles,mid,gate_values,HIDDEN,MID,(uint32_t)down_stride,SLOTS,down_type,&error)==FG_OK&&
        fg_vk_end(context,&error)==FG_OK&&
...
/* Production-shape decode pair timing: ten experts, gate/up 640x2560 and down
   2560x640, dispatched through the fused decode entry points that the ring
   block uses.  FG_BENCH_EXPERT_DECODE=<iterations> enables it; the printed
   rates are per-iteration GPU times over the expert weight bytes.  llvmpipe is
   instruction-bound, so this is an A/B proxy, not a blade bandwidth number. */
static int run_expert_decode_pair_bench(bool q8_down,uint32_t iterations){
    enum{EXPERTS=10,MID_ROWS=640,HIDDEN=2560};
    ...
    uint8_t *down_cooked=malloc((size_t)down_stride*EXPERTS);
    float input[HIDDEN],gates[EXPERTS];
    uint8_t q8[(HIDDEN/256u)*FG_Q8_K_BLOCK_BYTES];
    uint32_t tiles[EXPERTS*9u];
    int ok=raw&&gate_cooked&&up_cooked&&down_cooked;
    if(!ok)goto done;
    for(uint32_t expert=0;expert<EXPERTS;expert++){
        ...
        ok=fg_cook_k_quant_rows(gate_raw_expert,gate_cooked+(uint64_t)expert*gate_stride,gate_stride,HIDDEN,MID_ROWS,12u)&&
            ...
            (q8_down?fg_cook_q8_0_rows(down_raw_expert,down_cooked+(uint64_t)expert*down_stride,down_stride,MID_ROWS,HIDDEN):
             fg_cook_q5_1_rows(down_raw_expert,down_cooked+(uint64_t)expert*down_stride,down_stride,MID_ROWS,HIDDEN));
        if(!ok)goto done;
    }
    for(uint32_t i=0;i<HIDDEN;i++)input[i]=sinf((float)(i+3u)*0.013f)+0.1f*cosf((float)i*0.007f);
    fg_quantize_q8_k(input,q8,HIDDEN);
    for(uint32_t i=0;i<EXPERTS*9u;i++)tiles[i]=UINT32_MAX;
    for(uint32_t slot=0;slot<EXPERTS;slot++){
        tiles[slot*9u]=slot;
        tiles[slot*9u+1u]=slot;
        gates[slot]=0.6f-(float)slot*0.07f;
    }
    fg_vk_tensor *gw=tensor(gate_cooked,gate_stride*EXPERTS),*uw=tensor(up_cooked,up_stride*EXPERTS),
        *dw=tensor(down_cooked,down_stride*EXPERTS),*activation=tensor(q8,sizeof(q8)),
        *schedule=tensor(tiles,sizeof(tiles)),*gate_values=tensor(gates,sizeof(gates)),
        *mid=tensor(NULL,(uint64_t)EXPERTS*MID_ROWS*4u),*reduced=tensor(NULL,(uint64_t)HIDDEN*4u);
    ok=gw&&uw&&dw&&activation&&schedule&&gate_values&&mid&&reduced;
    ...
    fg_vk_profile gu={0},dr={0};
    if(ok)ok=fg_vk_profile_begin(context,&error)==FG_OK&&
        fg_vk_profile_set_scope(context,"expert_gate_up",&error)==FG_OK&&
        fg_vk_begin(context,&error)==FG_OK;
    for(uint32_t i=0;ok&&i<iterations;i++)
        ok=fg_vk_moe_decode_gate_up(context,mid,gw,uw,activation,schedule,MID_ROWS,HIDDEN,
            (uint32_t)gate_stride,(uint32_t)up_stride,12u,12u,EXPERTS,&error)==FG_OK;
    if(ok)ok=fg_vk_end(context,&error)==FG_OK&&fg_vk_profile_end(context,&gu,&error)==FG_OK;
    if(ok)ok=fg_vk_profile_begin(context,&error)==FG_OK&&
        fg_vk_profile_set_scope(context,"expert_down_reduce",&error)==FG_OK&&
        fg_vk_begin(context,&error)==FG_OK;
    for(uint32_t i=0;ok&&i<iterations;i++)
        ok=fg_vk_moe_decode_down_reduce(context,reduced,dw,schedule,mid,gate_values,HIDDEN,MID_ROWS,
            (uint32_t)down_stride,EXPERTS,q8_down?8u:7u,&error)==FG_OK;
    if(ok)ok=fg_vk_end(context,&error)==FG_OK&&fg_vk_profile_end(context,&dr,&error)==FG_OK;
    if(ok){
        double gu_ms=gu.kernel_ms/(double)iterations,dr_ms=dr.kernel_ms/(double)iterations;
        uint64_t gu_bytes=(uint64_t)EXPERTS*(gate_stride+up_stride),dr_bytes=(uint64_t)EXPERTS*down_stride;
        fprintf(stderr,"EXPERT_DECODE_BENCH q8_down=%d iterations=%u gu_gpu_us=%.3f dr_gpu_us=%.3f gu_GBps=%.1f dr_GBps=%.1f pair_gpu_us=%.3f\n",
            q8_down?1:0,iterations,gu_ms*1000.0,dr_ms*1000.0,
            (double)gu_bytes/gu_ms/1e6,(double)dr_bytes/dr_ms/1e6,(gu_ms+dr_ms)*1000.0);
    }
    fg_vk_tensor_destroy(reduced);fg_vk_tensor_destroy(mid);fg_vk_tensor_destroy(gate_values);
    fg_vk_tensor_destroy(schedule);fg_vk_tensor_destroy(activation);fg_vk_tensor_destroy(dw);
    fg_vk_tensor_destroy(uw);fg_vk_tensor_destroy(gw);
done:
    free(down_cooked);free(up_cooked);free(gate_cooked);free(raw);
    return ok;
}

static int test_expert_decode_pair_benchmark(void){
    const char *enabled=getenv("FG_BENCH_EXPERT_DECODE");
    if(!enabled||!*enabled)return 1;
    uint32_t iterations=(uint32_t)strtoul(enabled,NULL,10);
    if(!iterations)iterations=5u;
    int ok=run_expert_decode_pair_bench(false,iterations);
    ok=run_expert_decode_pair_bench(true,(iterations+1u)/2u)&&ok;
    return ok;
}

/* Random-routing parity for the batch-1 fused expert pair: random weights,
   gates, activation and slot maps (including dead slots) across the cooked
   Q4_K/Q5_K gate/up and Q5_1/cooked-Q8_0 down combinations, compared against
   the legacy five-dispatch path at <=1e-3 relative.  FG_EXPERT_PARITY_DUMP
   appends the fused rows so two kernel revisions can be diffed bit-for-bit. */
static uint32_t parity_rng(uint32_t *state){
    uint32_t x=*state;
...
static int run_expert_decode_fused_random(uint32_t seed,uint32_t gate_type,
    uint32_t up_type,uint32_t down_type,bool down_cooked){
    enum{HIDDEN=256,MID=64,SLOTS=10,EXPERTS=4};
    ...
    uint64_t down_raw=down_type==7u?(uint64_t)(MID/32u)*FG_Q5_1_BLOCK_BYTES*HIDDEN:
        (uint64_t)(MID/32u)*FG_Q8_0_BLOCK_BYTES*HIDDEN;
    uint8_t *gate_source=calloc(1,gate_raw*EXPERTS),*up_source=calloc(1,up_raw*EXPERTS),*down_source=calloc(1,down_raw*EXPERTS);
    uint8_t *gate_cooked=malloc(gate_stride*EXPERTS),*up_cooked=malloc(up_stride*EXPERTS),*down_cooked_data=malloc(down_stride*EXPERTS);
    float input[HIDDEN],expected[HIDDEN],got[HIDDEN],gates[SLOTS];
    uint8_t q8[FG_Q8_K_BLOCK_BYTES];
    uint32_t schedule[SLOTS*9u];
    ...
    int ok=gate_source&&up_source&&down_source&&gate_cooked&&up_cooked&&down_cooked_data;
    if(!ok)goto done;
    for(uint32_t expert=0;expert<EXPERTS;expert++){
        for(uint32_t row=0;row<MID;row++){
            uint32_t phase=parity_rng(&rng);
            if(gate_type==8u)
                make_q8_expert_row(gate_source+((uint64_t)expert*MID+row)*(HIDDEN/32u)*FG_Q8_0_BLOCK_BYTES,HIDDEN,phase);
            else
                make_k_row(gate_source+((uint64_t)expert*MID+row)*(HIDDEN/256u)*gate_block,gate_type==13u,phase);
            phase=parity_rng(&rng);
            if(up_type==8u)
                make_q8_expert_row(up_source+((uint64_t)expert*MID+row)*(HIDDEN/32u)*FG_Q8_0_BLOCK_BYTES,HIDDEN,phase);
            else
                make_k_row(up_source+((uint64_t)expert*MID+row)*(HIDDEN/256u)*up_block,up_type==13u,phase);
        }
        for(uint32_t row=0;row<HIDDEN;row++){
            uint8_t *destination=down_source+((uint64_t)expert*HIDDEN+row)*down_row;
            uint32_t phase=parity_rng(&rng);
            if(down_type==7u)make_q5_1_row(destination,MID,phase);
            else make_q8_expert_row(destination,MID,phase);
        }
    }
    for(uint32_t expert=0;ok&&expert<EXPERTS;expert++){
        if(gate_type==8u){
            if(!fg_cook_q8_0_rows(gate_source+(uint64_t)expert*gate_stride,gate_cooked+(uint64_t)expert*gate_stride,gate_stride,HIDDEN,MID))ok=0;
        }else if(!fg_cook_k_quant_rows(gate_source+(uint64_t)expert*gate_stride,gate_cooked+(uint64_t)expert*gate_stride,gate_stride,HIDDEN,MID,gate_type))ok=0;
        if(!ok)break;
        if(up_type==8u){
            if(!fg_cook_q8_0_rows(up_source+(uint64_t)expert*up_stride,up_cooked+(uint64_t)expert*up_stride,up_stride,HIDDEN,MID))ok=0;
        }else if(!fg_cook_k_quant_rows(up_source+(uint64_t)expert*up_stride,up_cooked+(uint64_t)expert*up_stride,up_stride,HIDDEN,MID,up_type))ok=0;
        if(!ok)break;
        if(down_type==7u&&!fg_cook_q5_1_rows(down_source+(uint64_t)expert*down_stride,down_cooked_data+(uint64_t)expert*down_stride,down_stride,MID,HIDDEN))ok=0;
        if(down_type==8u){
            if(down_cooked){
                if(!fg_cook_q8_0_rows(down_source+(uint64_t)expert*down_stride,down_cooked_data+(uint64_t)expert*down_stride,down_stride,MID,HIDDEN))ok=0;
            }else memcpy(down_cooked_data+(uint64_t)expert*down_stride,down_source+(uint64_t)expert*down_stride,(size_t)down_stride);
        }
    }
    for(uint32_t i=0;ok&&i<HIDDEN;i++)
        input[i]=sinf((float)(i+3u)*0.013f)+0.1f*cosf((float)(i+parity_rng(&rng)%17u)*0.007f);
    fg_quantize_q8_k(input,q8,HIDDEN);
    for(uint32_t i=0;i<SLOTS*9u;i++)schedule[i]=UINT32_MAX;
    for(uint32_t slot=0;ok&&slot<SLOTS;slot++){
        if((parity_rng(&rng)%4u)==0u)continue;
        schedule[slot*9u]=parity_rng(&rng)%EXPERTS;
        schedule[slot*9u+1u]=(slot*3u+seed)%SLOTS;
        gates[slot]=((float)(parity_rng(&rng)%2001u)-1000.0f)*0.001f;
    }
    if(ok){
        activation=tensor(q8,sizeof(q8));
        tiles=tensor(schedule,sizeof(schedule));
        gate_values=tensor(gates,sizeof(gates));
        gate=tensor(NULL,(uint64_t)SLOTS*MID*4u);
        up=tensor(NULL,(uint64_t)SLOTS*MID*4u);
        mid=tensor(NULL,(uint64_t)SLOTS*MID*4u);
    ...
    for(uint32_t i=0;ok&&i<HIDDEN;i++){
        ...
        if(relative>1e-3){
            fprintf(stderr,"expert_decode_fused_random seed=%u gate=%u up=%u down=%u cooked=%d row %u fused=%g legacy=%g rel=%g\n",
                seed,(unsigned)gate_type,(unsigned)up_type,(unsigned)down_type,(int)down_cooked,i,got[i],expected[i],relative);
            ok=0;
        }
    }
    ...
    if(ok&&dump&&*dump){
        ...
        if(file){
            fprintf(file,"seed=%u gate=%u up=%u down=%u cooked=%d",seed,(unsigned)gate_type,(unsigned)up_type,(unsigned)down_type,(int)down_cooked);
    ...
    if(ok&&seed<3u)
        fprintf(stderr,"expert_decode_fused_random seed=%u gate=%u up=%u down=%u cooked=%d max_rel=%.3g PASS\n",
            seed,(unsigned)gate_type,(unsigned)up_type,(unsigned)down_type,(int)down_cooked,max_rel);
done:
    fg_vk_tensor_destroy(fused);fg_vk_tensor_destroy(reference);fg_vk_tensor_destroy(down);
    fg_vk_tensor_destroy(mid);fg_vk_tensor_destroy(up);fg_vk_tensor_destroy(gate);
    fg_vk_tensor_destroy(gate_values);fg_vk_tensor_destroy(tiles);fg_vk_tensor_destroy(activation);
...
static int test_expert_decode_fused_random(void){
    int ok=1;
    for(uint32_t seed=0;seed<4u;seed++){
        ok=run_expert_decode_fused_random(seed,12u,13u,7u,false)&&ok;
        ok=run_expert_decode_fused_random(seed,13u,12u,7u,false)&&ok;
        ok=run_expert_decode_fused_random(seed,12u,13u,8u,true)&&ok;
        ok=run_expert_decode_fused_random(seed,12u,12u,8u,false)&&ok;
    }
    ok=run_expert_decode_fused_random(7u,8u,8u,8u,true)&&ok;
    return ok;
}
...
static int test_expert_graph_fused(int down_type,bool down_cooked){
    ...
    for(uint32_t expert=0;expert<EXPERTS;expert++){
        for(uint32_t row=0;row<MID;row++)
            make_k_row(gate_source+((uint64_t)expert*MID+row)*(HIDDEN/256u)*144u,0,expert*13u+row);
        for(uint32_t row=0;row<MID;row++)
            make_k_row(up_source+((uint64_t)expert*MID+row)*(HIDDEN/256u)*176u,1,expert*17u+row);
        for(uint32_t row=0;row<HIDDEN;row++){
            uint8_t *destination=down_source+((uint64_t)expert*HIDDEN+row)*down_row;
            if(down_type==7u){
                make_q5_1_row(destination,MID,expert*31u+row);
    ...
    for(uint32_t i=0;i<SLOTS*9u;i++)schedule[i]=UINT32_MAX;
    for(uint32_t slot=0;slot<SLOTS;slot++){
        schedule[slot*9u]=(slot+1u)%EXPERTS;
        schedule[slot*9u+1u]=(slot*3u+1u)%SLOTS;
        gates[slot]=0.6f-(float)slot*0.25f;
    }
    fg_vk_tensor *gw=NULL,*uw=NULL,*dw=NULL;
    fg_vk_tensor *activation=tensor(q8,sizeof(q8)),*tiles=tensor(schedule,sizeof(schedule)),*gate_values=tensor(gates,sizeof(gates));
    fg_vk_tensor *gate=tensor(NULL,(uint64_t)SLOTS*MID*4u),*up=tensor(NULL,(uint64_t)SLOTS*MID*4u),*mid=tensor(NULL,(uint64_t)SLOTS*MID*4u);
    fg_vk_tensor *down=tensor(NULL,(uint64_t)SLOTS*HIDDEN*4u),*reduced=tensor(NULL,(uint64_t)HIDDEN*4u);
    fg_vk_expert_graph *graph=NULL;
    int ok=activation&&tiles&&gate_values&&gate&&up&&mid&&down&&reduced;
    if(ok)ok=fg_vk_tensor_create(context,gate_stride*FG_EXPERTS_PER_RANK,&gw,&error)==FG_OK&&
        fg_vk_tensor_create(context,up_stride*FG_EXPERTS_PER_RANK,&uw,&error)==FG_OK&&
        fg_vk_tensor_create(context,down_stride*FG_EXPERTS_PER_RANK,&dw,&error)==FG_OK;
    if(ok)ok=fg_vk_tensor_write(gw,0,gate_cooked,gate_stride*EXPERTS,&error)==FG_OK&&
    ...
    if(ok)ok=fg_vk_begin(context,&error)==FG_OK&&
        ...
        fg_vk_end(context,&error)==FG_OK&&
        fg_vk_tensor_read(reduced,0,reference,sizeof(reference),&error)==FG_OK;
    if(ok)ok=fg_vk_expert_graph_create(context,&graph,activation,tiles,gate_values,gate,up,mid,down,reduced,gw,uw,dw,12u,13u,down_type,HIDDEN,MID,(uint32_t)gate_stride,(uint32_t)up_stride,(uint32_t)down_stride,EXPERTS,SLOTS,&error)==FG_OK;
    if(ok){
        fg_vk_counters before={0},after={0};
        fg_vk_get_counters(context,&before);
        ok=fg_vk_expert_graph_execute(graph,&error)==FG_OK&&
            fg_vk_tensor_read(reduced,0,got,sizeof(got),&error)==FG_OK;
        fg_vk_get_counters(context,&after);
        if(ok&&(after.submissions!=before.submissions+1u||after.dispatches!=before.dispatches+2u)){
            fprintf(stderr,"fused expert graph down=%d submissions=%llu dispatches=%llu\n",(int)down_type,
        ...
        for(uint32_t i=0;ok&&i<HIDDEN;i++)
            if(got[i]!=reference[i]){
                fprintf(stderr,"fused expert graph down=%d row %u replay=%g direct=%g\n",(int)down_type,i,got[i],reference[i]);
                ok=0;
            }
    }
    fg_vk_expert_graph_destroy(graph);
...
static int test_static_batch_replay(void){
    enum{VALUES=64,STEPS=4};float input[VALUES],bias[VALUES],got[VALUES];
    for(uint32_t i=0;i<VALUES;i++){input[i]=(float)i*0.0625f-1.0f;bias[i]=(float)(i%3u)*0.25f-0.5f;}
    fg_vk_tensor *si=tensor(input,sizeof(input)),*sb=tensor(bias,sizeof(bias)),*ya=tensor(NULL,sizeof(input)),*yb=tensor(NULL,sizeof(input));
    fg_vk_tensor *za=tensor(NULL,sizeof(input)),*zb=tensor(NULL,sizeof(input));
    int ok=si&&sb&&ya&&yb&&za&&zb;
    fg_vk_counters before={0},after={0};const fg_vk_tensor *current=si;
    if(ok)ok=fg_vk_static_begin(context,0,&error)==FG_OK;
    for(uint32_t step=0;ok&&step<STEPS;step++){fg_vk_tensor *target=(step&1u)?yb:ya;ok=fg_vk_add_f32(context,target,current,sb,VALUES,&error)==FG_OK;current=target;}
    if(ok)ok=fg_vk_static_end(context,0,&error)==FG_OK&&fg_vk_static_recorded(context,0);
    if(ok)ok=fg_vk_static_submit(context,0,&error)==FG_OK&&fg_vk_static_wait(context,0,&error)==FG_OK;
    if(ok)ok=fg_vk_tensor_read(yb,0,got,sizeof(got),&error)==FG_OK;
    for(uint32_t i=0;ok&&i<VALUES;i++)if(got[i]!=input[i]+(float)STEPS*bias[i])ok=0;
    float replacement[VALUES];
    for(uint32_t i=0;i<VALUES;i++)replacement[i]=0.5f-(float)i*0.125f;
    if(ok)ok=fg_vk_tensor_write(si,0,replacement,sizeof(replacement),&error)==FG_OK;
    if(ok)ok=fg_vk_static_submit(context,0,&error)==FG_OK&&fg_vk_static_wait(context,0,&error)==FG_OK;
    if(ok)ok=fg_vk_tensor_read(yb,0,got,sizeof(got),&error)==FG_OK;
    for(uint32_t i=0;ok&&i<VALUES;i++)if(got[i]!=replacement[i]+(float)STEPS*bias[i])ok=0;
    if(ok)ok=fg_vk_begin(context,&error)==FG_OK;
    current=si;
    for(uint32_t step=0;ok&&step<STEPS;step++){fg_vk_tensor *target=(step&1u)?zb:za;ok=fg_vk_add_f32(context,target,current,sb,VALUES,&error)==FG_OK;current=target;}
    if(ok)ok=fg_vk_end(context,&error)==FG_OK;
    fg_vk_get_counters(context,&before);
    if(ok)ok=fg_vk_static_submit(context,0,&error)==FG_OK&&fg_vk_static_drain(context,&error)==FG_OK;
    fg_vk_get_counters(context,&after);
    if(ok)ok=after.submissions-before.submissions==1u;
    if(ok)ok=fg_vk_tensor_read(yb,0,got,sizeof(got),&error)==FG_OK;
    for(uint32_t i=0;ok&&i<VALUES;i++)if(got[i]!=replacement[i]+(float)STEPS*bias[i])ok=0;
    if(ok)ok=fg_vk_static_begin(context,1,&error)==FG_OK;
    current=si;
    for(uint32_t step=0;ok&&step<STEPS;step++){fg_vk_tensor *target=(step&1u)?zb:za;ok=fg_vk_add_f32(context,target,current,sb,VALUES,&error)==FG_OK;current=target;}
    if(ok)ok=fg_vk_static_end(context,1,&error)==FG_OK;
    if(ok)ok=fg_vk_static_submit(context,0,&error)==FG_OK&&fg_vk_static_submit(context,1,&error)==FG_OK;
    if(ok)ok=fg_vk_static_drain(context,&error)==FG_OK;
    if(ok)ok=fg_vk_tensor_read(zb,0,got,sizeof(got),&error)==FG_OK;
    for(uint32_t i=0;ok&&i<VALUES;i++)if(got[i]!=replacement[i]+(float)STEPS*bias[i])ok=0;
    fg_vk_tensor_destroy(zb);fg_vk_tensor_destroy(za);fg_vk_tensor_destroy(yb);fg_vk_tensor_destroy(ya);fg_vk_tensor_destroy(sb);fg_vk_tensor_destroy(si);return ok;
}
...
ok=run_test("tensor_view_rebind",test_tensor_view_rebind)&&ok;
    ...
    ok=run_test("static_batch_replay",test_static_batch_replay)&&ok;
...
ok=run_test("expert_decode_pair_benchmark",test_expert_decode_pair_benchmark)&&ok;
...
ok=run_test_i("expert_decode_fused",test_expert_decode_fused,12)&&ok;
