...
OBJ := $(SRC:.c=.o)
DEP := $(OBJ:.o=.d) tests/test_core.d tests/test_chat.d tests/test_api.d tests/test_fg_vk.d tests/test_hc_down_split.d tests/test_model_load.d tests/test_tokenizer.d tests/test_fabric.d
...
tests/test_chat: tests/test_chat.c src/chat.c src/util.c include/fg_chat.h
	$(CC) $(CPPFLAGS) $(CFLAGS) -DFG_CHAT_RENDER_ONLY -o $@ tests/test_chat.c src/chat.c src/util.c $(LDLIBS)

tests/test_api: tests/test_api.c src/api.c src/chat.c src/util.c include/fg_api.h include/fg_chat.h
	$(CC) $(CPPFLAGS) $(CFLAGS) -DFG_CHAT_RENDER_ONLY -o $@ tests/test_api.c src/chat.c src/util.c $(LDLIBS)

test: tests/test_core tests/test_chat tests/test_api
	./tests/test_core
	./tests/test_chat
	./tests/test_api
...
clean:
	rm -f flash-gordon $(OBJ) $(DEP) tests/*.o tests/test_core tests/test_chat tests/test_api tests/test_fg_vk tests/test_hc_down_split tests/test_model_load tests/test_tokenizer tests/test_fabric vulkan/*.spv
