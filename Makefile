...
.PHONY: all clean test test-sampler test-embedding test-ngram-deployment test-vulkan test-qsa-fleet test-gdn-fleet test-pipeline-decode-fleet test_stage bench-bc250-roofline test-bc250-roofline shaders
all: flash-gordon
...
clean:
	rm -f flash-gordon $(OBJ) $(DEP) tests/*.o tests/test_core tests/test_session tests/test_prefix tests/test_chat tests/test_chat_runtime tests/test_api tests/test_sampler tests/test_embedding tests/test_ngram_deployment tests/test_fg_vk tests/bench_bc250_roofline tests/test_bc250_roofline tests/test_hc_down_split tests/test_model_load tests/test_qsa_model_load tests/test_tokenizer tests/test_fabric tests/test_pipeline tests/test_stage vulkan/*.spv
